...
title: 'KnitWit Mobile Runtime Architecture: Engine Selection and IR-to-GPU Playback Pipeline for Mid-Range Phones'
...
| Capability axis | Finding | Evidence |
| --- | --- | --- |
| Filament reach | Filament is a single-codebase real-time physically based rendering engine targeting Android, iOS, Linux, macOS, Windows, and WASM. | [claim:clm_045] |
| Filament backends | Filament's graphics backends span OpenGL 4.1+/ES 3.0+, Metal, Vulkan 1.0, WebGPU, and WebGL 2.0, covering desktop, mobile, and web targets. | [claim:clm_047] |
| Filament mobile posture | Filament is explicitly designed to be as small and efficient as possible on Android, signaling a lightweight mobile footprint posture relative to full game engines. | [claim:clm_046] |
| Filament maintenance | The latest Filament release is v1.71.6, published 2026-06-10, indicating active ongoing maintenance. | [claim:clm_049] |
| Filament glTF import | gltfio is a native glTF 2.0 loader for Filament, supporting compression and material extensions including KHR_draco_mesh_compression, KHR_materials_clearcoat, and KHR_materials_transmission. | [claim:clm_048] |
| RealityKit reach | RealityKit is supported on visionOS, iOS, macOS, and iPadOS, and as of WWDC25 it is newly supported on tvOS. | [claim:clm_004] |
| RealityKit asset model | RealityKit is designed around the open USD standard introduced by Pixar in 2012, whereas SceneKit serializes assets into proprietary SCN files that are not a unified industry standard. | [claim:clm_005] |
| RealityKit dynamic mesh | LowLevelMesh is a container for vertex data that lets developers create and update RealityKit meshes using their own custom vertex format. | [claim:clm_050] |
| RealityKit OS floor | LowLevelMesh requires iOS 18 / iPadOS 18 / macOS 15 (Sequoia) / visionOS 2 (with tvOS 26), setting the minimum deployment-target floor for any KnitWit feature that depends on it. | [claim:clm_051] |
| SceneKit status | Apple officially deprecated SceneKit across all platforms at WWDC25. | [claim:clm_001] |
| SceneKit maintenance mode | SceneKit is entering maintenance mode: Apple will only fix critical bugs, with no new features or optimizations expected going forward. | [claim:clm_003] |
| SceneView (Android) | SceneView is an Android view that integrates both ARCore and Google Filament for 3D and AR rendering. | [claim:clm_008] |
| SceneView stack | The canonical SceneView SDK is described as a Jetpack Compose + Filament 3D/AR SDK for Android (with iOS and Web variants). | [claim:clm_012] |
| Sceneform-successor status | Google archived Sceneform in 2021 and ships no first-party declarative AR renderer; SceneView descends from the maintained community fork and is the actively-developed, Jetpack-Compose-native way to build 3D/AR on Android. | [claim:clm_013] |
| Sceneform Maintained | Sceneform Maintained is an ARCore Android SDK that uses Google Filament as its 3D engine, and is presented as the continuation of the archived Sceneform project. | [claim:clm_056] |
| Sceneform Maintained release | The latest release is version 1.23.0 (dated 2023-08-04), with no newer releases, consistent with the stated 'not actively updated' status. | [claim:clm_060] |
| react-native-filament | react-native-filament is a React Native library for rendering 3D graphics in-app via React components, wrapping Google's Filament engine. | [claim:clm_021] |
| react-native-filament size | The native Filament dependency adds only about 4MB to the downloadable app size. | [claim:clm_022] |
| Thermion (Flutter) | Thermion is a framework for building cross-platform 3D applications with Dart and/or Flutter. | [claim:clm_038] |
| Thermion reach | Thermion targets iOS, macOS, Android, and Windows plus a Web/WASM build, giving broad mobile, desktop, and web reach from a single codebase. | [claim:clm_041] |
| Unity binary cost | A Unity universal (fat) build bundles a 32-bit and a 64-bit slice containing the same executable for two architectures, roughly doubling the binary size versus a single-architecture build. | [claim:clm_017] |
| Apple size limits | Apple enforces binary size limits that vary by MinimumOSVersion: 80 MB total __TEXT for <7.0, 60 MB per architecture slice for 7.x-8.x, and 500 MB for the Mach-O file for 9.0+. | [claim:clm_018] |

## Cross-platform layer option
...
## IR-to-GPU runtime data flow

### Import boundary: glTF/.glb to GPU buffers
...
### Per-round partial update: Filament setBufferAt and hot-swap
...
### Per-round partial update: RealityKit LowLevelMesh
...
### Per-round partial update: three.js BufferAttribute
...
## Performance budgets for a mid-range reference device
...
- What triangle/vertex ceiling per piece and per scene, draw-call ceiling, and texture-memory ceiling does a real mid-range reference device sustain at 30-60 FPS for a structural amigurumi stitch-graph mesh?
- Does surface-reconstruction meshing push polycount high enough to break the partial-update budget that setBufferAt/addUpdateRange/LowLevelMesh-Part assume?
- Is an iOS 18 deployment-target floor acceptable for KnitWit's launch, or does the iOS-17-or-earlier requirement force the Filament path on iOS as well?
