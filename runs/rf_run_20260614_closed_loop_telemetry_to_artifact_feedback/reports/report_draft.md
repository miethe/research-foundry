...
Scope: telemetry signals for automated artifact improvement, signal-to-revision lag, and governance structures for a system with immutable artifact versions behind a mutable pointer.
...
## Telemetry signal types and when loops degrade quality

### Leading signals: cheap, early, predictive
...
### Lagging signals: trustworthy but labor- and latency-bound
...
### When the loop degrades quality
...
| Signal | Class | Reliable-improvement evidence | Degradation condition | Evidence |
| --- | --- | --- | --- | --- |
| Tool-call provenance (tool, args, result, duration) | Leading | Per-trace provenance captured for every call | n/a (descriptive capture) | [claim:clm_006] |
| Per-trace cost/latency attribution | Leading | Localizes the sub-task consuming 80% of tokens or +3s latency | n/a (descriptive capture) | [claim:clm_007] |
| Activation-based reward-hack monitor | Leading | Distinguishes hacking from benign, generalizes to unseen adapters, earlier than output evals | Ground truth only GPT-4o-judged on 7B-8B models | [claim:clm_047] |
| Task chain length | Leading | Exploit propensity rises sharply at length 5 | Becomes a hazard past the produce-without-verification transition | [claim:clm_063] |
| Span-level human scores | Lagging | Localizes failure to retrieval, tool use, or generation | Labor-bound; value lost if not fed back | [claim:clm_029] |
| Human-reviewed failures as regression rows | Lagging | Each failure becomes a permanent regression test | Value trapped when review is a disconnected process | [claim:clm_030] |
| Quality-gate pass rate (LLM-judge) | Lagging | Embeds human-validated standards across CI and prod when calibrated | Self-preference bias and Goodhart overoptimization if uncalibrated | [claim:clm_032] |

## Lag: how long the loop should wait before acting
...
### Recommended lag by loop regime

| Loop regime | Method | Signal->revision lag rule | Power cost | Evidence |
| --- | --- | --- | --- | --- |
| Unpredictable-volume artifact, continuous stream | Always Valid Inference | Promote when the always-valid p-value crosses threshold, regardless of calendar | ~15% vs GAVI / ~30% vs GST under 50x underestimate | [claim:clm_inf05] |
| Predictable weekly batch, estimable sample size | Group Sequential Test | Promote at a pre-planned interim look once the GST boundary is crossed | Highest power even when sample size is overestimated | [claim:clm_inf06] |
| Few interim looks per version | Bonferroni-corrected z-test | Apply correction across the small fixed number of looks | Competitive with AVI at low look counts | [claim:clm_inf07] |
| Any metric checked repeatedly without correction | None (anti-pattern) | Do not promote on the first favorable reading | False-positive rate ~5% -> ~10% after two looks | [claim:clm_inf04] |

## Governance structures that keep the loop net-positive
...
### Reward hacking, exploit rates, and environment controls
...
### Judge bias as a governance failure mode
...
### Feedback-mechanism taxonomy context
...
### Governance-structure comparison matrix

| Structure | What it protects against | Evidence strength | Lag cost | Fit to immutable-artifact / mutable-pointer | Evidence |
| --- | --- | --- | --- | --- | --- |
| HITL checkpoint | Disconnected feedback; uncalibrated judges | Multiple primary sources, vendor-reported mechanism | Labor-bound, slowest | Human sign-off gates the pointer flip | [claim:clm_inf14] |
| Environmental hardening | Reward hacking / exploit at the grading boundary | Direct controlled experiment, 87.7% reduction, no task-success cost | Front-loaded, one-time before loop runs | Applied to the artifact's grader before any auto-revision | [claim:clm_inf08] |
| Sequential-test A/B promotion | Peeking-driven false-positive promotions | Peer-reviewed statistics, deployed at scale | Tunable via confidence threshold | Promotes a version only on confidence-qualified win | [claim:clm_inf05] |
| Holdout / champion-challenger | Regressions and unconfirmed wins | Inferential, analogical fit (not directly evidenced for agentic artifacts) | Holdout-traffic accrual time | Champion frozen, pointer flipped on win, rollback = pointer move | [claim:clm_inf13] |
...
## Minimal telemetry schema CCDash/SkillMeat must emit
...
| Field group | Concrete fields | Cardinality | Capture point | Status | Evidence |
| --- | --- | --- | --- | --- | --- |
| Artifact identity | artifact_id, version_id | Low | Dispatch (pointer-resolved) | version_id is new RF field | [claim:clm_inf15] |
| Tool-call provenance | tool name, args hash, result status, duration_ms | High (one row per call) | Per call | Already emitted in trace model | [claim:clm_006] |
| Cost/latency | input_tokens, output_tokens, cost_usd | Medium | Per model invocation | Already in per-trace signals | [claim:clm_008] |
| Quality gate | quality_gate_id, pass/fail, judge_score | Medium | Gate evaluation | Span-level scoring already supported | [claim:clm_029] |
| Human review | human_edit_count, rework_iteration | Low | At review | human_edit_count is new RF field | [claim:clm_030] |


## Analytical derivation: composing signals, lag, and gates

**Inference:** The leading/lagging split is the organizing axis: leading signals are cheap pre-outcome predictors and lagging signals are post-hoc confirmations, so the loop triages on the former and gates on the latter. [claim:clm_inf01]
**Inference:** The statistical layer then sets the lag: because re-checking a per-version metric is structurally identical to A/B peeking, the loop must apply a sequential correction sized to its number of interim looks rather than promote on first favorable reading. [claim:clm_inf04]
**Inference:** The method selection within that layer is parametric: continuous-stream artifacts take Always Valid Inference and predictable-batch artifacts take Group Sequential Tests, with Bonferroni reserved for low-peek loops. [claim:clm_inf05]
**Inference:** The governance layer wraps the statistics: environmental hardening is applied to the grader before any auto-revision, behavioral inoculation neutralizes the side effects of optimizing toward the gate, and chain-length routing forces human review past length 4. [claim:clm_inf08]
**Inference:** The judge-bias result closes the loop on the gate itself: an LLM-judge pass rate is only trustworthy after debiasing and human calibration, otherwise the loop optimizes a biased instrument. [claim:clm_inf03]
...
### Reward-model overoptimization as the formal warning
...
**Inference:** Gate promotion on lagging human-validated signals and use leading signals only to prioritize which artifacts to revise. [claim:clm_inf01]
**Inference:** Treat human-edit and rework counts as the primary quality-improvement signal because each human edit both fixes the artifact and calibrates the cheaper judge. [claim:clm_inf02]
**Inference:** Never let a raw LLM-judge pass rate drive revision; debias it with pairwise multi-dimensional scoring and pin it to human-calibrated ground truth first. [claim:clm_inf03]
**Inference:** Never promote on the first favorable reading; apply a sequential correction sized to the number of interim looks. [claim:clm_inf04]
**Inference:** Set lag by statistical confidence, not the calendar; use Always Valid Inference for streaming artifacts and accept the ~15-30% power cost of acting before sample size is known. [claim:clm_inf05]
**Inference:** Prefer Group Sequential Tests when revision cadence and per-version sample size are predictable. [claim:clm_inf06]
**Inference:** Use a Bonferroni-corrected z-test, not full sequential machinery, when the loop performs only a few interim looks per version. [claim:clm_inf07]
**Inference:** Harden the artifact's grading/verification environment before enabling any signal-driven auto-revision. [claim:clm_inf08]
**Inference:** Require agentic/process-level evals (span scores, tool-call traces) as the promotion criterion, not final-output scores alone. [claim:clm_inf09]
**Inference:** Add a mild inoculation prompt as a near-zero-cost behavioral layer complementing environmental hardening. [claim:clm_inf10]
**Inference:** Calibrate each MLOps-analogous trigger threshold empirically per artifact rather than importing a default. [claim:clm_inf11]
**Inference:** Bind each RIB-051 gate transition to a distinct signal class so automated advancement cannot conflate itself with the human gate. [claim:clm_inf12]
**Inference:** Adopt a champion-challenger / holdout structure so rollback is a single pointer move rather than a re-derivation. [claim:clm_inf13]
**Inference:** Force human review and lower the auto-promotion threshold for any artifact execution whose tool-call chain exceeds length 4. [claim:clm_inf17]
...
- What per-artifact numeric thresholds should trigger re-evaluation given that the MLOps guidance fixes no universal default?
- Does the activation-based reward-hack monitor remain reliable above the 7B-8B model scale at which it was validated?
- What KL-style distance budget keeps DSPy-style prompt optimization net-positive against a frozen champion?
- How much per-version traffic must accrue before an Always Valid Inference threshold crossing is trustworthy for low-volume artifacts?
