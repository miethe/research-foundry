...
title: What is the evidence-grounded design for a canonical,
...
**Inference:** The KnitWit Crochet IR should be a DSL surface over a graph core (hybrid), because three independent peer-reviewed sources (Digital Crochet, AmiGo, and Greer/Mould) all model crochet natively as a node-and-edge graph while CrochetPARADE's adoptable public-domain layer is itself a node/connection grammar, so a graph core gives a single validation substrate and a CrochetPARADE-style DSL gives the human-authoring and corpus-compatibility surface. [claim:clm_inf01]
**Inference:** Graph branching for shaping should be canonicalized as inc = one base insertion point with multiple feeding stitch nodes and dec = one stitch node consuming multiple base insertion points, because Digital Crochet, AmiGo's transducer, and Guo's stitch-mesh faces independently encode exactly this multiplicity, making it the cross-source-validated representation for the IR's increase/decrease semantics. [claim:clm_inf02]
**Inference:** The single-continuous-yarn / current-loop continuity invariant is the most strongly corroborated rule in the corpus, asserted in three forms (Seitz's no-use-before-create, AmiGo's row-coupling existence theorem, and Guo's current-loop edge), and therefore must be a hard, non-bypassable gate in the validator for both forward and inverse engines. [claim:clm_inf03]
...
**Speculation:** A bounded inc/dec-rate rule (e.g. no more than roughly a 2x stitch-count change per round, with tighter limits near high curvature) will likely be needed for amigurumi crochetability, but the reviewed corpus supplies only AmiGo's qualitative curvature-obstruction table and sampling-rate modulation rather than a numeric per-round cap, so the exact threshold must be derived empirically and is currently speculation. [claim:clm_spec01]
**Speculation:** If KnitWit ships the forward visualizer on the hybrid IR first and treats the AmiGo-style inverse mesh-to-pattern generator as a later experiment-gated phase, the IR's graph core plus coupling/continuity validator will likely be reusable unchanged across both engines, making the inverse generator a payoff add-on rather than an IR redesign. [claim:clm_spec02]
**Speculation:** Designer-defined / non-standard stitches will likely be the long-tail blocker for the corpus round-trip claim, because Guo's tile set covers only ch/ss/sc/sc2tog/sc3tog and Greer tested only sc-like stitches, so CrochetPARADE's DEF extensibility mechanism will probably have to carry most real-world stitch variety (dc, tr, post stitches) that the graph-layout literature has not yet validated. [claim:clm_spec03]
