...
title: 'Agentic web/scientific research tools: a mid-2026 capability landscape for Research Foundry discovery adapters'
...
This scan assesses four candidate agentic research tools as real-mode discovery adapters for Research Foundry (RF): GPT Researcher, Perplexity Sonar Deep Research, Gemini Deep Research, and PaperQA2. Only two of the four carry first-party evidence in this run.
...
The matrix below scores each tool against the dimensions that drive RF adapter selection.

| Dimension | GPT Researcher | Perplexity Sonar Deep Research | Gemini Deep Research | PaperQA2 |
| --- | --- | --- | --- | --- |
| API / invocation | Python class GPTResearcher(query=...) with conduct_research()/write_report() [claim:clm_032] | HTTP chat-completions plus async submit/poll endpoints (GET /v1/async/sonar 3,000 RPM, GET /v1/async/sonar/{request_id} 6,000 RPM) [claim:clm_041] | Evidence gap: no source card in this run [claim:clm_inf01] | Evidence gap: no source card in this run [claim:clm_inf01] |
| Headless API confirmed | Yes, headless via Python constructor and report-type API [claim:clm_inf02] | Yes, headless via async submit/poll endpoints and JSON response [claim:clm_inf02] | Evidence gap: no source card in this run [claim:clm_inf01] | Evidence gap: no source card in this run [claim:clm_inf01] |
| Output format | Prose report (PDF/Word/Markdown), APA citation style by default [claim:clm_021] | Structured search_results array (title, url, date, last_updated, snippet) [claim:clm_012] | Evidence gap: no source card in this run [claim:clm_inf01] | Evidence gap: no source card in this run [claim:clm_inf01] |
| RF schema fit | Lower: prose citations must be parsed out post hoc [claim:clm_inf03] | Higher: structured fields map field-for-field onto source_card [claim:clm_inf03] | Evidence gap: no source card in this run [claim:clm_inf01] | Evidence gap: no source card in this run [claim:clm_inf01] |
| Per-deep-run cost | ~$0.40 deep / ~$0.10 standard [claim:clm_022] | ~$0.816 in a worked example [claim:clm_014] | Evidence gap: no source card in this run [claim:clm_inf01] | Evidence gap: no source card in this run [claim:clm_inf01] |
| Pricing model | LLM/retriever provider passthrough; tunable via tier overrides [claim:clm_inf11] | $2/1M input, $8/1M output, $2/1M citation, $3/1M reasoning, $5/1K search [claim:clm_011] | Evidence gap: no source card in this run [claim:clm_inf01] | Evidence gap: no source card in this run [claim:clm_inf01] |
| Governance / license | Apache 2.0: perpetual, royalty-free, irrevocable, no automated-use ToS [claim:clm_inf04] | Hosted-only; every call traverses Perplexity servers under a tier-gated account [claim:clm_inf05] | Evidence gap: no source card in this run [claim:clm_inf01] | Evidence gap: no source card in this run [claim:clm_inf01] |
| Throughput limits | Self-hosted, bounded only by configured providers [claim:clm_inf05] | Tier 0 sonar-deep-research capped at 5 RPM, rising to 100 RPM at Tier 5 [claim:clm_039] | Evidence gap: no source card in this run [claim:clm_inf01] | Evidence gap: no source card in this run [claim:clm_inf01] |
| Data residency | Self-hostable via OPENAI_BASE_URL for in-region routing [claim:clm_inf05] | Hosted-only, no in-region routing option [claim:clm_inf05] | Evidence gap: no source card in this run [claim:clm_inf01] | Evidence gap: no source card in this run [claim:clm_inf01] |
| API stability | Multiple minor releases per quarter; one breaking change in v3.3.7 [claim:clm_inf09] | 128K-token context, stable hosted API surface [claim:clm_010] | Evidence gap: no source card in this run [claim:clm_inf01] | Evidence gap: no source card in this run [claim:clm_inf01] |

## Derivation and analysis

This section reasons from the matrix to the adapter decision, distinguishing the structured-output, governance, cost, and throughput axes.
...
The following recommendation and the rules beneath it operationalize the analysis into adapter sequencing and implementation guardrails.

**Inference:** Implement GPT Researcher as the first real-mode RF adapter and Perplexity Sonar Deep Research as the second: GPT Researcher wins on governance (Apache 2.0, self-hostable, no automated-use ToS), cost-floor ($0.10 standard / $0.40 deep), and a native Python API, while Perplexity ranks second for its cleaner structured-JSON output but loses on hosted-only data residency and tier-gated throughput; Gemini Deep Research and PaperQA2 are deferred until source cards exist. [claim:clm_inf10]

Decision rules derived from the analysis:
...
- What is Gemini Deep Research's API availability, output format, pricing, and governance posture once a dedicated source card exists?
- What is PaperQA2's API availability, output format, pricing, and governance posture once a dedicated source card exists?
- Do the Perplexity per-deep-run cost figures and tier thresholds hold beyond the single worked example captured in this run?
