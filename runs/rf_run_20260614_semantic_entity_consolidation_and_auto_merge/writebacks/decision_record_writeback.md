---
id: mwb_20260622_dr_semantic_entity_consolidation_and_auto
evidence_bundle_id: bundle_20260615_intent_research_20260614_semantic_entity_consoli
target_page: meatywiki/decisions/semantic_entity_consolidation_and_auto_merge.md
writeback_type: decision_record
status: written
summary: 'Decision record from run rf_run_20260614_semantic_entity_consolidation_and_auto_merge: EnsembleLink
  and Splink both achieve high accuracy with zero labeled data (clm_062/clm_064/clm_071), which matches
  a per'
key_claims:
- claim_id: clm_inf01
  include: true
...
# Decision Record: Semantic entity consolidation and auto-merge thresholds in a file-first vault
...
- EnsembleLink and Splink both achieve high accuracy with zero labeled data (clm_062/clm_064/clm_071), which matches a personal KB that lacks a labeled duplicate corpus; LLM re-rank (clm_007/clm_042) is the highest-accuracy but costliest tier, so reserving it for the review band mirrors the ensemble-then-rerank recommendation in clm_060. [claim:clm_inf01]
- clm_002 gives the Balanced 0.95/0.85 profile and the three-tier structure comes from clm_001; Splink clusters at 0.95 (clm_075) and dedupe defaults to 0.5 cophenetic (clm_053), so 0.95 auto-merge is the convergent high-precision cut, with the flag floor raised toward 0.80 to widen the review band for irreversible-loss safety. [claim:clm_inf05]
- clm_003 establishes the embedding-dominant blended score (0.7/0.3) and same-type gating; clm_037 makes bidirectional links/graph a first-class Obsidian signal and clm_032 makes source-card provenance available, so the blend is widened to add provenance and graph terms while keeping body semantics dominant. Exact weights are an untuned proposal. [claim:clm_inf06]
- clm_033 shows every blob/commit is SHA-named and history is self-verifying; clm_034 shows the derived index is deterministically rebuildable, so a merge committed as one Git commit is atomically revertible and the index can be rebuilt afterward, satisfying the transaction/rollback requirement (clm_032). [claim:clm_inf08]
- clm_004 supplies SAME_AS metadata (confidence/status/timestamps/reviewed_by) reusable as the redirect-stub frontmatter; clm_036/clm_037 establish that any tool can rewrite Markdown links and that links are first-class, enabling backlink rewrite + redirect stubs; clm_034 confirms the derived index is rebuilt from files post-merge. [claim:clm_inf09]
- clm_037 makes backlink density a meaningful authority signal for choosing the surviving title; clm_032 makes provenance lists first-class and unionable to preserve evidence; clm_001's three-tier model justifies escalating irreconcilable conflicts to review rather than silently merging. [claim:clm_inf10]
- clm_063 specifies dense+sparse union retrieval for high-recall candidate generation and clm_065 adds hierarchical fuzzy blocking robust to misspellings; clm_022 shows the traditional alternative (exact blocking/sorted-neighbourhood) and clm_028/clm_069 show this family scales to ~1M records and stays stable across corpus sizes, so it comfortably covers 100K. [claim:clm_inf11]
- clm_047-050 describe dedupe's active-learning loop (uncertain_pairs/mark_pairs/train) that learns the classifier and blocking rules from human-labeled uncertain pairs; the three-tier review band (clm_001) is precisely where uncertain pairs land, so feeding those labels back closes the calibration loop. [claim:clm_inf14]
- clm_005 supplies the exact healthy ranges (rejection <2%, flag-to-confirm 60-90%, orphan <10%) and the corrective action of raising the auto threshold; combined with the active-learning loop (clm_inf14) these metrics become the feedback controller that adjusts the domain-tuned bands (clm_002) at runtime. [claim:clm_inf15]
- clm_042 is ComEM's medium-LLM-filter then powerful-LLM-identify-top-k pipeline and clm_045 shows it cuts cost while gaining F1; clm_069/clm_063 show the cheap stage can run locally with no API calls, so restricting the costly LLM to the small review band is the cost-optimal allocation. [claim:clm_inf16]
- clm_007 gives GPT-4 zero-shot 89.82 on DBLP-Scholar and clm_067 gives EnsembleLink 89.0 (tau=0.8) on the same dataset, approaching supervised Ditto ~94.3; the gap is small and supervised methods require thousands of labels (clm_009), which a single-user KB cannot cheaply produce. [claim:clm_inf02]
- clm_011 documents sharp PLM degradation (Ditto 36-56%, RoBERTa 22-61% drops) on out-of-distribution entities and a 34% GPT-4 advantage; a personal KB continuously ingests novel topics, so its entity distribution is permanently out-of-distribution relative to any fixed training set, making robustness (clm_006) the decisive criterion. [claim:clm_inf03]
- clm_061 explicitly states pairwise matching approaches a ceiling and effort should shift to blocking/clustering/review; clm_057 and clm_069 corroborate 98-99% F1 ceilings, so additional scorer tuning yields diminishing returns versus pipeline components. [claim:clm_inf04]
- clm_003 only matches same-type entities to suppress false positives; by analogy provenance/graph overlap (available per clm_032/clm_037) serves as an additional structural gate, so shared provenance confirms and disjoint provenance vetoes, treating these signals as confidence modifiers on top of text similarity. [claim:clm_inf07]
- clm_028 reports ~1M records/minute on a laptop and clm_029 reserves Spark/Athena for 100M+; 100K is 10x below the laptop figure, and clm_069 runs locally on small open models, so the dominant marginal cost shifts to embedding each artifact (clm_063's dense retrieval stage) rather than the blocked comparisons. [claim:clm_inf12]
- clm_013 makes lambda a function of duplicate prevalence and clm_016 folds lambda into the prior weight log2(lambda/(1-lambda)); a low-duplicate vault lowers the prior, shifting the whole weight distribution down, so per clm_079 the threshold must be raised to preserve precision under imbalance. [claim:clm_inf13]
- clm_069 emphasizes fixed open-source weights give deterministic reproducible results with no API calls; clm_010 (no single best prompt; per-model/dataset tuning) plus hosted model drift make GPT-4 outputs non-reproducible, conflicting with the SHA-based auditability of clm_033, so the deterministic local model is preferred for the standing scorer. [claim:clm_inf17]

## Consequences

- Recommended auto-merge / review-queue / ignore bands for compiled artifacts, expressed on the blended 0-1 confidence scale: auto-merge at >=0.95, human review at 0.80-0.95, and ignore (new entity) below 0.80 -- anchoring on the Ecommerce-Balanced profile but biased toward precision because a wrong auto-merge silently destroys a unique note. [claim:clm_inf05]
- The Neo4j 70/30 embedding/fuzzy blend should be extended to a four-signal score for vault artifacts -- approximately body cosine 0.50, title similarity 0.20, provenance (source-card) overlap 0.20, graph-neighborhood (backlink) overlap 0.10 -- because compiled notes carry provenance and link structure that bare entity records lack. [claim:clm_inf06]
- Git content-addressing supplies the merge-transaction and rollback primitive a files-as-truth vault needs: each merge is one atomic commit, content-SHA naming makes the pre-merge state tamper-evidently recoverable, and `git revert` is the reversible-merge undo -- so no bespoke transaction log is required. [claim:clm_inf08]
- A safe reversible-merge for Markdown-as-truth should: (1) stage all edits then commit as one transaction, (2) rewrite inbound wikilinks/backlinks to the surviving note, (3) leave a redirect-stub note at the merged-away path carrying a SAME_AS-style pointer, and (4) rebuild the derived index from files -- making the operation auditable and undoable via the SAME_AS metadata plus the commit. [claim:clm_inf09]
- Survivorship should be field-level, not record-level: keep the title with the higher in-vault backlink count, union (not overwrite) the provenance/source-card lists and tags, prefer the longer-provenance body as the golden record, and route any genuinely conflicting scalar frontmatter (e.g. divergent status) to the human-review queue rather than auto-picking. [claim:clm_inf10]
- To stay sub-quadratic to 100K artifacts, candidate generation should use the EnsembleLink-style union of dense-embedding ANN retrieval and sparse character-n-gram (TF-IDF) blocking, optionally wrapped in hierarchical (fuzzy-on-blocking-variable) blocking, rather than Splink-style exact blocking keys, because artifact titles are misspelling- and paraphrase-prone. [claim:clm_inf11]
- The review queue should be calibrated by dedupe-style active learning (uncertain_pairs -> human label -> retrain) rather than fixed bands, because human decisions on the most uncertain artifact pairs are exactly the labels that retune the scorer and the band edges over time, converting the unavoidable review effort into model improvement. [claim:clm_inf14]
- Operational health metrics should gate the bands: keep auto-merge rejection (false-auto-merge) rate below 2% by raising the auto threshold if breached, target a 60-90% flag-to-confirm rate to keep the review band well-calibrated, and watch orphan/redirect-stub rate below ~10% -- these act as the closed-loop controller for the otherwise-static thresholds. [claim:clm_inf15]
- The cost/quality decision rule is tiered by band: run the cheap local zero-shot scorer (dense+sparse retrieve, small open cross-encoder) over all candidate pairs, and invoke an expensive proprietary LLM (GPT-4o/ComEM selecting) only on the review band's top-k -- mirroring ComEM's filter-then-identify pipeline that gained 2-18% F1 while spending less. [claim:clm_inf16]
- On reported pair-level F1, zero-shot LLM and zero-shot retrieve-rerank methods are statistically comparable to each other and to supervised PLMs on bibliographic data: GPT-4 zero-shot 89.82 and EnsembleLink 89.0 on DBLP-Scholar versus supervised Ditto ~94, so the ~4-5 F1 supervised premium does not justify the labeling cost for a personal vault. [claim:clm_inf02]
- Because fine-tuned PLM matchers collapse 22-61% F1 on unseen entities while GPT-4 zero-shot stays robust and beats the best transferred PLM by 34% F1, supervised dedup models should be rejected for an open-ended personal vault whose artifact vocabulary drifts continuously. [claim:clm_inf03]
- Pairwise matching is near a practical ceiling (best LLMs ~98-99% F1 on large benchmarks), so the marginal engineering payoff for a file-first vault lies not in a better pair scorer but in blocking, clustering, and the human-review/uncertainty loop. [claim:clm_inf04]
- Provenance and graph-neighborhood overlap act as a precision multiplier rather than a primary signal: two artifacts compiled from the same source cards or sharing dense backlinks should have their merge confidence boosted into the auto band, while disjoint provenance should cap confidence at the review band even at high body cosine, mirroring same-type gating. [claim:clm_inf07]
- Splink alone proves blocking-plus-scoring can link ~1M records on a laptop in ~1 minute, so a 100K-artifact vault is comfortably within single-machine, no-big-data-backend reach; the constraining cost is embedding generation, not pair comparison. [claim:clm_inf12]
- A personal KB has far lower duplicate prevalence than the balanced ER benchmarks (where positives are engineered to ~ tens of percent), so the prior match probability lambda is small; this class imbalance means the optimal operating point should sit higher than benchmark-tuned thresholds, because even a modest false-positive rate dominates the rare true matches. [claim:clm_inf13]
- Determinism and reproducibility favor a fixed-weight local model (EnsembleLink on Qwen3-Embedding-0.6B + Jina reranker) over hosted GPT-4-class APIs for the always-on scorer, because hosted LLM results drift across API snapshots whereas pinned open weights give byte-stable, Git-auditable merge decisions consistent with files-as-truth. [claim:clm_inf17]
