...
The strongest entity-resolution methods now reach a practical accuracy ceiling on pairwise matching, with the best LLMs scoring ~98-99% F1 on large benchmarks. [claim:clm_061]

**Inference:** Because pairwise matching is near a practical ceiling, the marginal engineering payoff for a file-first vault lies not in a better pair scorer but in blocking, clustering, and the human-review/uncertainty loop. [claim:clm_inf04]
...
## Candidate methods for entity resolution
...
### Method 1: Zero-shot LLM matching (GPT-4 class)
...
### Method 2: Fellegi-Sunter probabilistic linkage (Splink)
...
### Method 3: Zero-shot retrieve-and-rerank (EnsembleLink)
...
### Method 4: Compound LLM pipeline (ComEM)
...
### Method 5: Active-learning deduplication (dedupe)
...
### Method 6: Classical record-linkage toolkit (Python Record Linkage Toolkit)
...
### Large-scale and rule-based ensembling evidence
...
This matrix scores the candidate methods on precision, recall, blocking scalability, and reversibility fit for a file-first vault. Each data row carries its evidence tag.

| Method | Precision signal | Recall signal | Blocking scalability | Reversibility fit (file-first) | Evidence |
|--------|------------------|---------------|----------------------|--------------------------------|----------|
| Zero-shot LLM (GPT-4 class) | F1 95.78 Abt-Buy, 98.41 DBLP-ACM; beats fine-tuned PLMs on product datasets | F1 89.82 DBLP-Scholar, 89.67 Walmart-Amazon zero-shot | No native blocking; pairwise LLM cost dominates at scale | n/a | [claim:clm_007] |
| Zero-shot LLM robustness | Robust on unseen entities where PLMs collapse 22-61% F1; GPT-4 beats transferred PLM by 34% F1 | Robust to unseen entities | n/a | n/a | [claim:clm_011] |
| Fellegi-Sunter / Splink | Unsupervised match score per pair; precision raised by threshold | Recall raised by lowering threshold | ~1M records on a laptop in ~1 minute | n/a | [claim:clm_028] |
| Fellegi-Sunter / Splink scale | n/a | n/a | 100M+ records via Spark/AWS Athena backends | n/a | [claim:clm_076] |
| EnsembleLink (retrieve-rerank) | 89.0 pair-level F1 at tau=0.8 DBLP-Scholar; 97.9% top-1 accuracy, no training data | High-recall dense+sparse candidate retrieval | Accuracy stable 0.98-0.99 from 5,000 to 64,263 records | Runs locally, deterministic reproducible results | [claim:clm_067] |
| EnsembleLink determinism | n/a | n/a | n/a | Local small open models, no API calls, deterministic reproducible results | [claim:clm_069] |
| ComEM (compound LLM) | Improves precision without sacrificing recall; F1 87.62 Abt-Buy, 90.85 DBLP-ACM | Retains selecting strategy's high recall; 84.68 DBLP-Scholar | Filter-then-identify reduces cost vs single-strategy LLM | n/a | [claim:clm_045] |
| dedupe (active learning) | Pairwise classifier from labeled pairs; partition() trades recall vs precision at cophenetic threshold (default 0.5) | Cluster admission tunable via threshold | Learns fingerprinting (blocking) rules from training data | n/a | [claim:clm_053] |
| Python Record Linkage Toolkit | Jaro-Winkler / Damerau-Levenshtein string comparison with similarity thresholds | Blocking + sorted neighbourhood candidate generation | Indexing methods (blocking, sorted neighbourhood) on pandas/numpy | n/a | [claim:clm_022] |
| Nomenklatura (rule-based) | 91.33% F1; over-predicts matches | High-recall filtering candidate | Rule-based, scales to 755,540-pair / 1M-entity benchmark | n/a | [claim:clm_057] |
...
## Analytical derivation: scoring and thresholds

### Fellegi-Sunter match-weight algebra
...
### Threshold partitioning and the precision/recall tradeoff
...
### Empirical threshold and signal-mix profiles
...
## Safe, reversible merge design for a file-first vault
...
## Sub-quadratic blocking strategy to 100K
...
- Will multi-agent RF/MeatyWiki swarms be the first workload to breach the file-first single-writer breakpoint, forcing per-agent branch+merge isolation or a PostgreSQL-backed write coordinator before the corpus itself grows large? [claim:clm_038]
- **Speculation:** Will multi-agent RF/MeatyWiki swarms running auto-merge concurrently breach the file-first single-writer ceiling before the corpus size does, forcing per-agent branch+merge isolation or a merge-coordinator queue so that two agents never rewrite the same backlink graph in the same instant? [claim:clm_spec02]
- How current is the Splink threshold-selection guidance, given the post was originally published on 2020-04-16 and last updated 2022-08-04? [claim:clm_077]
