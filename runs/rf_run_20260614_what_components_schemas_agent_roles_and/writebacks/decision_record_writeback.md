---
id: mwb_20260622_dr_minimum_viable_architecture_for_an
evidence_bundle_id: bundle_20260614_intent_research_20260614_what_components_schemas
target_page: meatywiki/decisions/minimum_viable_architecture_for_an_evidence.md
writeback_type: decision_record
status: written
summary: 'Decision record from run rf_run_20260614_what_components_schemas_agent_roles_and: Two-level
  parallelism gives up to 90% time savings (clm_057) at ~15x token cost (clm_054); for an MVP whose governance
  i'
key_claims:
- claim_id: clm_inf08
  include: true
...
# Decision Record: Minimum Viable Architecture for an Evidence-Backed Research Swarm
...
- Two-level parallelism gives up to 90% time savings (clm_057) at ~15x token cost (clm_054); for an MVP whose governance is a runtime budget gate (clm_025, clm_026), the prudent default is low/no fan-out, adding parallelism only once budget controls are proven. [claim:clm_inf08]
- FActScore scores atomic facts against a knowledge source (clm_007, clm_010) and ALCE measures citation recall/precision (clm_066) with a 50%-incomplete-support finding (clm_067). RF's source-card-relative four-way labeling (clm_022) maps directly onto these metrics, so they are the natural quantitative test of whether RF's claims are verifiable. [claim:clm_inf09]
- RF's spec already maps GPT Researcher and PaperQA2 to MVP roles (clm_026); both speak LiteLLM/OpenAI-compatible APIs (clm_020, clm_050, clm_044) and expose agentic loops (clm_014), so wrapping them as carders/retrievers is lower-risk than rebuilding retrieval and citation. [claim:clm_inf10]
- The real 0.1 artifact already ties every finding to a claim id and source card (clm_034, clm_036, clm_037); ALCE supplies precision/recall (clm_066) and GPT Researcher gives a concrete per-run cost reference (~$0.4, clm_049), yielding four measurable functional-swarm criteria. [claim:clm_inf13]
- GPT Researcher (planner/execution/publisher), PaperQA2 (Paper Search/Gather Evidence/Generate Answer), STORM (Knowledge Curation/Outline/Article Generation/Polishing), and Anthropic (lead/subagents) all instantiate plan-retrieve-synthesize; none of the five carries a dedicated adversarial red-team role, so that role is not load-bearing for a minimal functional swarm. [claim:clm_inf01]
- Two independent production systems (Anthropic, PaperQA2) externalize attribution as an explicit mechanism rather than trusting inline LLM citation, and RF's claim-id-terminated sentences plus source-card-linked ledger are the same idea; therefore traceability machinery, not just prose citations, is essential for a claim-traceable swarm. [claim:clm_inf02]
- Anthropic explicitly tiers a strong lead over cheaper parallel subagents and reports a 90.2% gain; PaperQA2 keeps one mid-tier model across roles. RF's principle of routing extraction to cheap models and synthesis to strong models is the same lever the 90.2% result attributes performance to, so the divergence from PaperQA2's single-tier default is a justified design choice. [claim:clm_inf03]
- If token usage explains 80% of variance and multi-agent runs use ~15x chat tokens, then cost and quality both scale primarily with tokens; RF already maps LiteLLM to cost-based routing with budgets, so capping token spend per provider/model/tag is the dominant control, more than picking a cheaper model. [claim:clm_inf04]
- GPT Researcher summarizes and source-tracks resources (clm_046), STORM produces article-level citations (clm_038), and Anthropic/PaperQA2 attach citations to passages (clm_056, clm_017), but none of the cards describes an explicit inference/speculation labeling partition; RF's four-way labeling (clm_022) is therefore a justified divergence that exceeds prior-art traceability. [claim:clm_inf05]
- RF's durable bundle is source cards + claims + ledger + report (clm_023); prior art universally has evidence units (clm_046, clm_018) and attribution + report (clm_056). Writeback to MeatyWiki/SkillMeat/CCDash (clm_026) is RF-specific with no prior-art analog, so it is deferrable while the three core schemas are not. [claim:clm_inf06]
- Anthropic documents a concrete context-window failure (clm_055) that any long-horizon swarm shares. RF's durable bundle (clm_023) persists artifacts but the MVP CLI surface and tool map (clm_026) do not name a plan/state memory; this is a foreseeable gap RF should close by persisting the plan into the bundle. [claim:clm_inf07]
- GPT Researcher's discrete steps (clm_046) and PaperQA2's three tools (clm_014) both reduce to retrieve->summarize->attribute->generate; RF's prioritized CLI (clm_027) plus claim-id markers (clm_034) implement the same loop, with init/writeback being setup/output adjuncts rather than loop-critical. [claim:clm_inf11]
- No surveyed system has a voting review gate; the closest analogs are Co-STORM's Moderator (clm_041) and Anthropic's CitationAgent (clm_056). RF's Council (clm_026) plus runtime governance (clm_025) make it valuable where output is governed, but its absence everywhere else suggests it can be deferred for personal-scope runs. [claim:clm_inf12]
- PaperQA2 leads on paper retrieval and citation (clm_029, clm_017); GPT Researcher gives cheap broad web reports (clm_048, clm_049); Anthropic's orchestration gives the largest quality lift (clm_052) at the largest token cost (clm_054), so workload type determines the cost-justified choice. [claim:clm_inf14]
