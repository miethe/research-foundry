...
title: Minimum Viable Architecture for an Evidence-Backed Research Swarm
...
Across all five comparable systems (GPT Researcher, PaperQA2, STORM/Co-STORM, Anthropic Research, OpenAI Deep Research), a planner/decomposer role, one or more retrieval-execution roles, and a synthesis/aggregation role are universal, so RF's researcher and synthesizer roles are MUST-have while a standalone red-team role appears in none of them and is therefore CAN-defer for MVP. **Inference:** [claim:clm_inf01]
The MUST-have RF execution loop is capture-sources -> extract-claims-into-cards -> map-claims-to-ledger -> synthesize-report -> verify-gate, which is the common denominator of GPT Researcher's plan/crawl/summarize/aggregate and PaperQA2's search/gather/generate; the rf init and rf writeback commands are loop-adjacent and CAN-defer relative to capture/plan/verify. **Inference:** [claim:clm_inf11]
An evidence/source-card schema, a claim-ledger schema, and a report schema are the three MUST-have RF schemas, because every surveyed system materializes evidence units (chunks/contexts/sources), an attribution mapping (citation keys or a citation pass), and a final report, whereas RF's writeback targets (MeatyWiki/SkillMeat/CCDash) have no analog in any comparable system and are therefore safely CAN-defer post-MVP. **Inference:** [claim:clm_inf06]
RF's choice to make the claim ledger the authority (every material claim maps to a source card or is labeled inference/speculation) is the strongest traceability discipline among the surveyed systems: GPT Researcher only source-tracks summaries, STORM cites at the article level, and even Anthropic and PaperQA2 attach citations to passages but do not enforce a four-way support/inference/speculation/unresolved partition. **Inference:** [claim:clm_inf05]

## Minimum viable architecture specification

### Design principles carried from the RF spec
...
### Components and schemas: MUST-have vs CAN-defer

| Element | Category | MVP decision | Evidence |
|---|---|---|---|
| Evidence/source-card schema | Schema | MUST-have — every surveyed system materializes evidence units | [claim:clm_inf06] |
| Claim-ledger schema | Schema | MUST-have — the four-way labeling authority | [claim:clm_022] |
| Report schema | Schema | MUST-have — every surveyed system emits a final report | [claim:clm_inf06] |
| Four-way claim labeling (supported/inference/speculation/unresolved) | Discipline | MUST-have — strongest traceability vs prior art | [claim:clm_inf05] |
| Plan/state persistence (context memory) | Component | MUST-have — closes the context-window truncation gap | [claim:clm_inf07] |
| Post-research citation/attribution step | Component | MUST-have — converges with CitationAgent and pqac keys | [claim:clm_inf02] |
| Verify-gate with atomic-fact + citation precision/recall metrics | Component | MUST-have — quantitative test of claim verifiability | [claim:clm_inf09] |
| Cheap-extract / expensive-synthesize model tiering | Routing | MUST-have — validated by the 90.2% Opus+Sonnet lift | [claim:clm_inf03] |
| Per-run LiteLLM provider/model/tag budget gate | Governance | MUST-have — the dominant cost-control lever | [claim:clm_inf04] |
| Standalone adversarial red-team role | Agent role | CAN-defer — appears in none of the five systems | [claim:clm_inf01] |
| Parallel subagent fan-out | Optimization | CAN-defer — highest leverage but ~15x token cost | [claim:clm_inf08] |
| Writeback targets (MeatyWiki/SkillMeat/CCDash) | Output | CAN-defer — no analog in any comparable system | [claim:clm_inf06] |
| rf init and rf writeback commands | CLI | CAN-defer — loop-adjacent, not loop-critical | [claim:clm_inf11] |
| Agent Council (Approve/Concern/Block voting) | Review | MUST-have for governed output, CAN-defer for personal runs | [claim:clm_inf12] |

### Agent roles

| Role | RF status | Cross-system basis | Evidence |
|---|---|---|---|
| Planner / decomposer | MUST-have | Universal across all five surveyed systems | [claim:clm_inf01] |
| Researcher (retrieval-execution) | MUST-have | Universal across all five surveyed systems | [claim:clm_inf01] |
| Synthesizer (aggregation) | MUST-have | Universal across all five surveyed systems | [claim:clm_inf01] |
| Citation/attribution agent | MUST-have | Mirrors Anthropic CitationAgent and PaperQA2 pqac keys | [claim:clm_inf02] |
| Agent Council reviewer | Conditional | Analog of Co-STORM Moderator and Anthropic CitationAgent check | [claim:clm_inf12] |
| Standalone red-team | CAN-defer | Absent from all five surveyed systems | [claim:clm_inf01] |

### Execution loop

The MUST-have RF execution loop is capture-sources -> extract-claims-into-cards -> map-claims-to-ledger -> synthesize-report -> verify-gate, which is the common denominator of GPT Researcher's plan/crawl/summarize/aggregate and PaperQA2's search/gather/generate; the rf init and rf writeback commands are loop-adjacent and CAN-defer relative to capture/plan/verify. **Inference:** [claim:clm_inf11]
RF's biggest unjustified gap relative to prior art is the absence of an explicit context-persistence/memory mechanism in the MVP: Anthropic's LeadResearcher must save its plan to Memory because exceeding the 200,000-token window truncates and loses it, so a deep RF swarm that decomposes a query across many source cards will hit the same truncation failure mode unless a plan/state persistence step is treated as MUST-have. **Inference:** [claim:clm_inf07]
...
### Citation and factuality benchmarks
...
## Validation and challenge analysis

### Role convergence across systems

| System | Planner/decomposer | Retrieval-execution | Synthesis/aggregation | Evidence |
|---|---|---|---|---|
| GPT Researcher | planner agents | execution agents | publisher aggregation | [claim:clm_045] |
| PaperQA2 | agentic loop ordering | Paper Search + Gather Evidence | Generate Answer | [claim:clm_014] |
| STORM | Knowledge Curation | perspective-driven conversation | Article Generation + Polishing | [claim:clm_039] |
| Anthropic Research | lead agent | parallel subagents | lead synthesis + CitationAgent | [claim:clm_051] |
| OpenAI Deep Research | single-prompt planning | hundreds of source reads | analyst-level report | [claim:clm_071] |

### Justified divergences and unjustified gaps

| RF design choice | Verdict | Basis | Evidence |
|---|---|---|---|
| Four-way claim labeling as authority | Justified divergence — exceeds prior-art traceability | GPT Researcher source-tracks summaries, STORM cites at article level, Anthropic/PaperQA2 cite passages | [claim:clm_inf05] |
| Cheap-extract / expensive-synthesize tiering | Justified divergence from PaperQA2 single-tier default | Generalizes Anthropic's Opus-lead/Sonnet-subagent 90.2% lift | [claim:clm_inf03] |
| Agent Council voting review | Justified divergence — no system has a voting gate | Closest analogs are Co-STORM Moderator and Anthropic CitationAgent | [claim:clm_inf12] |
| Post-research citation/attribution step | Convergent MUST-have, not novel | Anthropic CitationAgent and PaperQA2 pqac keys | [claim:clm_inf02] |
| Absence of context-persistence/memory in MVP | Unjustified gap | Anthropic LeadResearcher saves plan to Memory to survive the 200k-token window | [claim:clm_inf07] |
| Reusing PaperQA2/GPT Researcher vs reimplementing | Recommended reuse | Both are LiteLLM/OpenAI-compatible and Apache-2-or-open | [claim:clm_inf10] |

### Cost and performance derivation

The single largest cost driver for any RF deep run is token volume, not model selection or orchestration overhead, because Anthropic found token usage alone explained 80% of BrowseComp performance variance and that multi-agent systems consume ~15x the tokens of a chat, implying RF's per-run budget gates (LiteLLM provider/model/tag budgets) are the correct primary cost-control lever. **Inference:** [claim:clm_inf04]
RF's cheap-extract/expensive-synthesize split is validated by prior art rather than novel: it generalizes Anthropic's Opus-4-lead / Sonnet-4-subagent tiering and PaperQA2's single-tier gpt-4o-2024-11-20 default, and the Anthropic finding that an Opus+Sonnet split beat single-agent Opus by 90.2% is direct evidence that role-based model tiering improves quality per dollar. **Inference:** [claim:clm_inf03]
Parallel fan-out of subagents should be deferred past RF's first MVP even though it is the highest-leverage performance feature, because Anthropic's two-level parallelism cut research time up to 90% but multi-agent runs cost ~15x chat tokens, so a sequential or low-fan-out loop is the right MVP default and parallelism is a CAN-defer optimization gated behind budget enforcement. **Inference:** [claim:clm_inf08]
...
A functional RF swarm can be demonstrated with four concrete metrics — claim coverage (percent of material report sentences terminating in a claim id, target 100%), source-card support rate (percent of supported claims with at least one resolving source card), citation precision/recall on a held-out sample, and cost-per-run under the LiteLLM budget — and the existing 0.1 artifact already proves claim coverage and support-rate are mechanically enforceable. **Inference:** [claim:clm_inf13]

## Recommendations and decision rules

A dedicated post-research citation/attribution step is a MUST-have rather than a nice-to-have, because Anthropic runs a separate CitationAgent after the research loop and PaperQA2 embeds citation keys (pqac-abcd1234) into every answer, converging on the same pattern RF encodes as its claim ledger plus claim-id markers of the form clm_NNN. **Inference:** [claim:clm_inf02]
RF's verification gate should adopt FActScore-style atomic-fact decomposition and ALCE-style citation precision/recall as its concrete claim-verification metrics, because both define support relative to a given knowledge source (not global truth), which is exactly RF's source-card-relative model, and ALCE shows even the best models lack complete citation support 50% of the time on ELI5 — quantifying the failure RF's ledger is designed to catch. **Inference:** [claim:clm_inf09]
RF should reuse PaperQA2 and GPT Researcher as execution components rather than reimplement their loops, because PaperQA2 already provides agentic RAG with in-text citations over papers and GPT Researcher already provides cited web/hybrid reports, both are LiteLLM/OpenAI-compatible and Apache-2-or-open, matching RF's LiteLLM routing principle exactly. **Inference:** [claim:clm_inf10]
On a quality-per-dollar ranking for RF's likely workloads, PaperQA2 is the best fit for scientific-paper questions (superhuman LitQA2 retrieval, native citation keys), GPT Researcher is the best fit for open-web/hybrid reports at ~$0.4/run, and an Anthropic-style Opus+Sonnet orchestration is the best fit only for high-stakes synthesis where the ~15x token premium is justified by the 90.2% quality lift. **Inference:** [claim:clm_inf14]
RF's Agent Council (Approve/Concern/Block voting) review stage is a justified divergence from prior art that none of the five systems implements, and it is best positioned as the human-in-the-loop analog of Co-STORM's Moderator role and Anthropic's CitationAgent check — making it MUST-have for governed/work output but CAN-defer for low-stakes personal runs. **Inference:** [claim:clm_inf12]

### Workload routing decision table

| Workload | Executor | Rationale | Evidence |
|---|---|---|---|
| Scientific-paper questions | PaperQA2 | Superhuman LitQA2 retrieval, native citation keys | [claim:clm_inf14] |
| Open-web / hybrid reports | GPT Researcher | Cited web/hybrid output at ~$0.4/run | [claim:clm_inf14] |
| High-stakes synthesis | Anthropic-style Opus+Sonnet orchestration | ~15x token premium justified by 90.2% quality lift | [claim:clm_inf14] |
| Governed/work output review | Agent Council voting gate | Human-in-the-loop analog of Moderator/CitationAgent | [claim:clm_inf12] |
| Low-stakes personal runs | Agent Council deferred | Voting gate absent from all five surveyed systems | [claim:clm_inf12] |

## Open questions

- As MCP-connected deep-research products (OpenAI deep research connecting to any MCP/app, GPT Researcher's hybrid Tavily+MCP retriever) become the default, will RF's durable evidence-bundle and claim-ledger shift from being the system that generates research to being the governance/audit layer that wraps externally-run MCP research swarms? **Speculation:** [claim:clm_spec01]
- Will RF's claim ledger plausibly need a contradiction-tracking field promoted to MUST-have sooner than its spec assumes, because PaperQA2/ContraCrow find an average of 2.34 contradictions per biology paper and STORM editors flagged over-association of unrelated facts, suggesting cross-source contradiction is common enough that an MVP without it will surface unflagged conflicts? **Speculation:** [claim:clm_spec02]
- If RF enforces atomic-fact-level verification on every material claim, will its dominant MVP cost migrate from synthesis to verification, because FActScore's automated estimator approximates human scoring at <2% error but still required scoring 6,500 generations (a study that would have cost ~$26K under human evaluation), implying per-claim verification at scale is the line item most likely to break a naive per-run budget? **Speculation:** [claim:clm_spec03]
