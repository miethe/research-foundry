...
title: 'AI-Assisted Crochet Pattern Parsing: On-Device vs Cloud Capability Review'
...
This review characterizes the crochet pattern parsing problem and grades current AI extraction capability against the KnitWit operating spec's amigurumi-first mobile constraint.

The deterministic core of pattern parsing is a finite token-to-op normalization plus a count/repeat expander rather than an open-ended NLP problem, because the CYC abbreviation list and CrochetPARADE's compact syntax map almost one-to-one onto the Crochet IR v0.1 op enum. [claim:clm_inf02]
For clean machine-readable pattern text, a rule-based tokenizer plus a deterministic stitch-count validator is the highest-accuracy and lowest-cost extractor available and should own the parse path, because the IR's expected_stitch_count field provides a closed-form correctness check no current model matches on procedural fidelity. [claim:clm_inf03]
The on-device-vs-cloud boundary for document/layout parsing falls between text-layout extraction (cloud-grade today) and on-device deployment, because the strongest open document parsers publish only server-side cost and none is documented to run within a mid-range phone's memory budget. [claim:clm_inf04]
Stitch-chart and symbol-diagram parsing is the hardest sub-problem and the clearest cloud-only boundary, because chart understanding requires visual-comparison reasoning where the best open model reaches only 38.5% and even the best proprietary model 63.0% versus 93% human. [claim:clm_inf07]
The viable KnitWit import architecture is a three-tier cascade of on-device deterministic tokenizer, on-device small VLM or cloud OCR, and cloud VLM reserved for charts and ambiguous diagrams. [claim:clm_inf08]
Academic feasibility is established for OCR/layout transcription but product-readiness for an amigurumi-first mobile importer is not, because no source documents a mid-range-phone latency/memory figure, crochet-specific extraction accuracy, or chart-symbol recognition above the ChartMuseum ceiling. [claim:clm_inf15]
...
## Capability evidence: structured extraction from documents and charts

### Document-layout and OCR vision-language models
...
### Chart and symbol-diagram understanding
...
### Domain procedural synthesis (crochet-specific)
...
### Structured-output and mesh-to-pattern precedents
...
CrochetPARADE exporting to SVG produces multiple files, including a chart using standard crochet symbols (optionally with yarn colors) and a stitch-connection/view diagram. [claim:clm_081]
All CrochetPARADE computation runs locally on the user's device, with no data collected to a server or transmitted over the internet. [claim:clm_082]
...
## On-device vs cloud decision frame for mobile

The candidate-model footprint table below maps documented size, latency, memory, and accuracy figures onto the mid-range-phone constraint, with each material row carrying its evidence tag.

| Model | Params / encoder | Latency (reported) | Memory | Doc accuracy | Deployment side | Evidence |
| --- | --- | --- | --- | --- | --- | --- |
| PaddleOCR-VL | 0.9B; NaViT encoder + ERNIE-4.5-0.3B | 1.2241 pages/s on A100 | ~43.7 GB GPU | OmniDocBench 92.86 | Server-side only | [claim:clm_005] |
| dots.mocr | 3B VLM on 1.7B LLM | None reported | Not reported | olmOCR-Bench 83.9 | Unquantified on-device | [claim:clm_069] |
| MagicVL-2B | 93M ViT encoder + Qwen3-1.7B | 0.09s ViT + 1.7s LLM, 23.9 tok/s | n/a | DocVQA 89.0, OCRBench 828 | Flagship smartphone (Snapdragon 8 Elite) | [claim:clm_044] |
| SmolVLM (2B) | SmolLM2 1.7B backbone | None on-phone | 5.02 GB GPU RAM | DocVQA 81.6 | On-device memory floor | [claim:clm_022] |
| Apple on-device model | ~3B, 2-bit QAT, KV-cache sharing | TTFT cut ~37.5% | KV-cache cut 37.5% | Beats Qwen-2.5-3B on MMLU | On-device (Apple silicon) | [claim:clm_071] |
...
## Analytical derivation: a viable parsing architecture
...
The IR-field reliability matrix appears below.

| IR field | Maps from | Extractable today? | Evidence |
| --- | --- | --- | --- |
| pattern.terminology (us/uk) | Declared locale context resolving US/UK token collision | Requires explicit context resolution before op decoding | [claim:clm_inf01] |
| ops[].op (mr/ch/slst/sc/hdc/dc/tr) | CYC core stitch tokens | Reliable for clean text | [claim:clm_030] |
| ops[].op dec | CYC 2tog decrease tokens | Reliable for clean text | [claim:clm_031] |
| ops[].repeat | Standardized asterisk/bracket repeat notation | Reliable for clean text | [claim:clm_032] |
| ops[].count / expected_stitch_count | Compact integer-multiplied stitch syntax | Reliable and closed-form checkable | [claim:clm_inf03] |
| visual_hint.shape_role | Shaping/termination cues under-specified in text | Ambiguous; image-grounded cues are weak | [claim:clm_013] |
| assembly[].attachment_points | Chart geometry / pragmatic free-text notes | Ambiguous; below chart-reasoning ceiling | [claim:clm_inf11] |
...
- What is the documented latency and memory footprint of MagicVL-2B-class on-device VLMs on a mid-range (non-flagship) phone SoC rather than a Snapdragon 8 Elite?
- What crochet-pattern-specific extraction accuracy do document-layout parsers achieve, given that no source reports a crochet-domain evaluation?
- Can any approach recognize stitch charts and symbol diagrams above the ~38-63% ChartMuseum visual-reasoning ceiling for crochet symbols specifically?
