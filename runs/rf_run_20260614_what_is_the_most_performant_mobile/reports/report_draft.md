...
title: 'KnitWit Stitch-Highlight Rendering: Overlay Technique and Incremental-Update Benchmark for Mobile Playback'
...
# KnitWit Stitch-Highlight Rendering: Overlay Technique and Incremental-Update Benchmark for Mobile Playback

## Executive summary

**Inference:** For amigurumi-scale per-stitch highlight playback, three.js BatchedMesh ranks first on the combined draw-call/update/fidelity axis because setColorAt rewrites only the advancing round's per-instance colors while the whole piece stays a single batched draw, beating per-vertex vertex-color groups, naive per-object draws, and a separate overlay pass. [claim:clm_inf01]
**Inference:** The recommended highlight technique is per-stitch instanced color (BatchedMesh.setColorAt over an opaque single-material batch), with the ghost-next-row drawn as a separate small transparent pass limited to only the next round's instances, so the opaque body preserves early-ZS/FPK overdraw removal while transparency cost is bounded to one thin blended layer. [claim:clm_inf02]
**Inference:** The dominant mid-range-phone bottleneck for this workload is per-pixel blended-layer overdraw, not draw-call count, so the 30-60 FPS budget breaks first when stacked transparent ghost/overlay layers raise per-pixel blended-layer count rather than when stitch count rises. [claim:clm_inf03]
**Inference:** The instanced per-stitch approach sits on the product-ready side for the forward highlight/ghost-row visualizer, whereas full yarn/fiber fidelity sits on the academic-feasibility-only side because the SIGGRAPH 2025 real-time knit method runs only on a desktop RTX 3090 at 23.6 ms/frame with no mobile numbers. [claim:clm_inf10]

## Technique comparison matrix

| Technique | Draw-call count (row-advance) | Per-row update cost | Fidelity / overdraw behavior | Evidence |
|-----------|-------------------------------|---------------------|------------------------------|----------|
| Per-vertex vertex-color groups | Advancing one round forces a partial vertex-buffer (VAO) re-upload, which makes the browser revalidate and recalculate fetch limits, the weakest path for incremental playback. | Drawing from static, unchanging VAOs is faster than mutating a VAO per draw because browsers cache fetch limits for unchanged VAOs but must revalidate when VAOs change. | Recoloring writes the geometry's color attribute and mutates the VAO/its buffers, so it pays the revalidation cost on every row-advance. | [claim:clm_inf04] |
| Per-stitch GPU instancing (BatchedMesh) | The whole piece stays a single batched draw while setColorAt rewrites only the advancing round's per-instance colors. | BatchedMesh is a multi-draw batch-rendering mesh for many objects sharing one material but differing geometries or transforms, reducing draw calls to improve rendering performance. | setColorAt assigns a per-instance color (a Vector4 can define alpha) and getColorAt reads it back, enabling per-stitch or per-round highlights against a static geometry buffer. | [claim:clm_011] |
| Index-range sub-draws | Each highlighted round becomes its own draw call, so draw-call count grows linearly with the number of distinct highlight states rather than staying at one batched draw. | Index-range sub-draws (one drawElements per advancing round over a contiguous index range) are a viable middle option for highlight-only playback but lose to instanced color. | Texture atlasing and degenerate triangles let discontinuous geometry stay in a single drawArrays(TRIANGLE_STRIP) batch, which index-range sub-draws forgo by splitting per round. | [claim:clm_inf05] |
| Separate overlay pass (blended) | Adds extra blended draws on top of the opaque body. | Splitting large UI elements into separate opaque and transparent portions lets early ZS or FPK remove the overdraw beneath the opaque parts that blending would otherwise prevent. | Blending disables key overdraw-removal optimizations on Arm GPUs, specifically early ZS testing and Forward Pixel Kill, which is especially costly for UIs and 2D games with many sprite layers. | [claim:clm_006] |
...
Arm's guidance favors batching geometry and avoiding many small draw calls to reduce per-draw CPU and tiling/binning overhead. [claim:clm_048]
...
## BatchedMesh capability surface
...
## Mobile-GPU cost model (why blended overdraw is the binding constraint)
...
### Overdraw-removal hardware that blending defeats
...
### Fragment Prepass recovery is small and generation-gated
...
### Adreno (Qualcomm) considerations
...
**Inference:** Per-vertex vertex-color groups are the weakest mapping technique for incremental playback because advancing one round forces a partial vertex-buffer (VAO) re-upload, which makes the browser revalidate and recalculate fetch limits, whereas instanced per-instance color updates a separate instance buffer and leaves the geometry VAO static. [claim:clm_inf04]
...
**Inference:** On Arm Mali/Immortalis the cost difference between an opaque instanced highlight and a blended overlay is structural, not marginal: blending disables early-ZS and FPK, and the Fragment Prepass only recovers single-digit-percent GPU-cycle savings (4.6-6.5% on real game scenes) and only on 2024-2025 GPUs, so older mid-range Mali phones get no prepass relief and pay full overdraw for every stacked transparent layer. [claim:clm_inf06]

## Academic feasibility vs product readiness

The instanced per-stitch approach sits on the product-ready side for the forward highlight/ghost-row visualizer (built from shipped, documented three.js BatchedMesh APIs and vendor mobile-GPU guidance), whereas full yarn/fiber fidelity sits on the academic-feasibility-only side. [claim:clm_inf10]
...
## IR identity and round-trip reuse

**Inference:** Per-stitch IR identity is preserved across playback and the inverse round-trip by mapping each Crochet IR op (mr|ch|sc|hdc|dc|tr|inc|dec within pieces[].rounds[].ops[]) to a stable BatchedMesh instance index, since getColorAt/setColorAt are index-addressed and setMatrixAt is per-instance, and the same instance-ID-to-op map then drives both forward row-highlight playback and reverse mesh-to-round visualization. [claim:clm_inf08]
...
## Contradictions and open disagreements

**Speculation:** A contradiction exists between three.js issue #31935 (instanced-multidraw deprecated, current emulation ~1.5-2x slower) and the recommendation to rely on BatchedMesh per-instance color, likely resolved by noting the slowdown is desktop-measured (n=1, iGPU/dGPU) and concerns multi-geometry multidraw throughput, not the per-instance color-update path this design uses; decision impact: medium, pending the mobile benchmark. [claim:clm_spec02]
...
### Reusable mobile benchmark protocol
...
**Inference:** The single biggest unproven risk for productizing instanced per-stitch highlights is LOD/decimation identity loss (severity: high; likelihood: medium): none of the gathered sources document that per-stitch instance IDs survive mesh decimation, so a mitigation is to keep the per-stitch instance layer separate from any decimated background shell and to LOD only the non-identity-bearing geometry, preserving the instance-to-IR-op map at every LOD. [claim:clm_inf09]
**Speculation:** An opaque single-material BatchedMesh stitch piece (~600-1500 instances) plus exactly one thin transparent ghost-next-row pass will hold 60 FPS on pre-2024 mid-range Mali/Adreno phones, and the 30-60 FPS budget will only break if the ghost/overlay design stacks roughly 3+ overlapping blended layers per pixel rather than from stitch count alone. [claim:clm_spec01]

## Prototype experiments and decision-gate relevance

The instanced per-stitch approach sits on the product-ready side for the forward highlight/ghost-row visualizer (built from shipped, documented three.js BatchedMesh APIs and vendor mobile-GPU guidance), so it most directly de-risks G3 (Pattern-to-3D viability) and the EXP-006 row-highlight-export experiment. [claim:clm_inf10]
Per-stitch IR identity is preserved across playback and the inverse round-trip by mapping each Crochet IR op to a stable BatchedMesh instance index, which supports G2 (Crochet IR viability) and the EXP-004 IR-to-stitch-graph and EXP-010 round-trip-evaluator experiments that depend on a stable instance-to-op map. [claim:clm_inf08]
A reusable mobile stitch-mesh highlight benchmark protocol fixing test mesh, device class, and overdraw/frame-time metrics is the concrete experiment that de-risks the EXP-012 mobile-rendering-feasibility gate ahead of the G5 MVP recommendation. [claim:clm_inf07]
The LOD/decimation identity-loss risk is the open item that the round-highlight-export (EXP-006) and round-trip-evaluator (EXP-010) experiments must validate before G3 and G4 can pass for product use. [claim:clm_inf09]

## Open questions

- Do per-stitch BatchedMesh instance IDs survive mesh decimation / LOD switches in any documented configuration?
- At what measured stitch count and blended-layer count does the recommended approach break the 30-60 FPS budget on a specific pre-2024 mid-range Mali device?
- Does the instanced-multidraw deprecation slowdown affect the per-instance color-update path on mobile GPUs, or only multi-geometry multidraw throughput on desktop?
- What is the measured per-row update cost of setColorAt on a real Adreno FlexRender device when the frame switches between binning and direct mode?
