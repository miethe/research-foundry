...
title: 'Cheap-extract vs deep-synthesize: the empirical cost/quality case for Research Foundry''s two-tier model split'
...
## Per-stage cost and accuracy comparison

The table maps each RF pipeline stage to the cheap tier, the deep tier, and the two-tier split, grounding the accuracy column in published evals and the cost column in published list pricing.

| Stage | Cheap-tier accuracy signal | Deep-tier accuracy signal | Two-tier split verdict | Evidence |
|-------|----------------------------|---------------------------|------------------------|----------|
| Structured extraction (leaf-value) | Phi-4 (14B) scored 0.798 Value Accuracy on text, slightly above GPT-5 (0.795) and GPT-5 Mini (0.779). | GPT-5 (0.795) frontier baseline on the same exact-match metric | Cheap tier is the default; differential is near-zero or negative | [claim:clm_029] |
| Structured extraction (leaf-value) | Schematron-8B scored 0.754, outperforming GPT-OSS 20B (0.693) despite having roughly 2.5x fewer parameters. | Larger open-weight GPT-OSS 20B underperforms at 0.693 | Smaller model wins; size does not buy accuracy here | [claim:clm_030] |
| Structured extraction (leaf-value) | The smaller open model Qwen3.5-35B scored 0.801 Value Accuracy on text, exceeding closed-source frontier models on the primary metric. | Closed-source frontier models trail on Value Accuracy | Cheap/open tier leads on the primary metric | [claim:clm_033] |
| Field-level extraction (25 typed fields) | Reflexive architectures achieved the highest field-level F1 (0.943) but at 2.3x the cost of sequential baselines. | Reflexive deep loop tops F1 at 0.943 but costs 2.3x | Deep-on-everything overpays for marginal F1 | [claim:clm_043] |
| Field-level extraction (hybrid routing) | Hybrid configurations (semantic caching, model routing, adaptive retry) recovered 89% of the reflexive architecture's accuracy gains at only 1.15x baseline cost. | Reflexive deep loop is the 100% reference at 2.3x cost | Hybrid (cheap default + escalation) dominates the frontier | [claim:clm_044] |
| Long-form synthesis | Across an evaluation of 23 LLMs, even state-of-the-art models encounter significant challenges in long-form generation as real-world complexity and output length increase. | Same fragility hits SOTA models; deep tier still strongest available | Reserve deep tier for this stage | [claim:clm_007] |
| Schema-constrained agentic work (general) | The paper claims guided decoding, strict JSON Schema outputs, and validator-first tool execution let SLMs match or surpass larger models on tool use, function calling, and RAG at 10x-100x lower token cost with better latency and energy. | Larger models match but do not beat on these constrained tasks | Cheap tier at 10x-100x lower token cost | [claim:clm_035] |

## Per-tier list pricing (cost basis)

The cost side of the comparison rests on published per-million-token (MTok) list prices, summarized below; the cheap-to-deep ratio derived from these rows drives the break-even analysis.

| Tier / model | Input ($/MTok) | Output ($/MTok) | Notes | Evidence |
|--------------|----------------|-----------------|-------|----------|
| Cheap (Anthropic) — Claude Haiku 4.5 | 1 | 5 | Cache reads at $0.10/MTok | [claim:clm_013] |
| Mid (Anthropic) — Claude Sonnet 4.5 / 4.6 | 3 | 15 | Both Sonnet versions identical | [claim:clm_014] |
| Deep (Anthropic) — Claude Opus 4.5-4.8 | 5 | 25 | All four Opus versions identical | [claim:clm_015] |
| Cheap floor (Google) — Gemini 3.1 Flash-Lite | 0.25 | 1.50 | Cheapest current-generation model, text/image/video | [claim:clm_023] |
| Deep (Google) — Gemini 3.1 Pro Preview | 2.00 | 12.00 | Rises to $4.00/$18.00 for prompts >200K tokens | [claim:clm_020] |
| Mid (Google) — Gemini 3.5 Flash | 1.50 | 9.00 | Output includes thinking tokens | [claim:clm_021] |
| Mid (Google) — Gemini 3 Flash Preview | 0.50 | 3.00 | Text/image/video input | [claim:clm_022] |
...
## Discount and tokenizer adjustments to the cost basis

| Modifier | Effect on cost | Evidence |
|----------|----------------|----------|
| Anthropic Batch API | The Batch API applies a 50% discount on both input and output tokens, e.g. Haiku 4.5 at $0.50/$2.50 and Sonnet at $1.50/$7.50. | [claim:clm_017] |
| Google Batch API | Google offers a Batch API with a 50% cost reduction relative to standard synchronous pricing across Gemini models. | [claim:clm_026] |
| Prompt caching | Prompt-caching multipliers relative to base input are 1.25x (5-min write), 2x (1-hour write), and 0.1x (cache read/hit). | [claim:clm_016] |
| Opus 4.7+ tokenizer | Opus 4.7 and later use a new tokenizer that may consume up to 35% more tokens for the same fixed text, affecting effective per-text cost. | [claim:clm_019] |
| Worked Haiku volume datapoint | Processing 10,000 support tickets averaging ~3,700 tokens each on Claude Haiku 4.5 costs roughly $37.00 total. | [claim:clm_018] |

## Analysis and break-even derivation

**Inference:** The deep-to-cheap price ratio sets the ceiling on what a split can save, and the per-run extraction volume sets where the split first clears its own overhead. [claim:clm_inf17]
...
### Supporting eval and pricing detail
...
- Below what exact per-run extraction token volume does the two-tier split stop paying for its routing and verifier overhead?
- What is the input-vs-output token split inside the support-ticket datapoint, given that the break-even derivation currently assumes a blended rate?
- Does a Gemini Flash-Lite extractor paired with an Opus synthesizer actually clear the same schema-validity bar as an all-Anthropic split when no source benchmarks the two providers head-to-head?
- What confidence threshold calibrates the cheap-tier-to-deep-tier escalation trigger without over-escalating into the deep tier?
