---
id: mwb_20260622_dr_contradiction_detection_across_heterogeneous
evidence_bundle_id: bundle_20260615_intent_research_20260614_contradiction_detection
target_page: meatywiki/decisions/contradiction_detection_across_heterogeneous_sou.md
writeback_type: decision_record
status: written
summary: 'Decision record from run rf_run_20260614_contradiction_detection_across_heterogeneous_sou: AlignScore-355M
  and MiniCheck-FT5-770M reach GPT-4-level consistency scoring at 400x lower cost (clm_031,078,079); Claim'
key_claims:
- claim_id: clm_inf03
  include: true
- claim_id: clm_inf04
...
# Decision Record: Contradiction detection across heterogeneous source cards: a technical memo
...
- AlignScore-355M and MiniCheck-FT5-770M reach GPT-4-level consistency scoring at 400x lower cost (clm_031,078,079); ClaimPKG (clm_008) and GraphJudge (clm_056,061) both validate the architectural pattern of a cheap specialized model feeding an expensive general model, so the same split applies to contradiction arbitration. [claim:clm_inf03]
- MiniCheck frames scoring as a thresholded binary classifier at t=0.5 (clm_081) and reports ~30 min for 29K pairs on one A6000 (clm_083); combined with the GPT-4-parity-at-low-cost result (clm_031,079) this yields a cheap-gate-then-escalate break-even where only sub-threshold pairs reach the expensive arbiter. [claim:clm_inf04]
- ClaimPKG (clm_008) retrieves only relevant subgraphs rather than the whole KG, and the inter-context task (clm_044,072) inherently scales with document/card count; applying similarity-based blocking before pairing is the standard sub-quadratic reduction implied by that retrieval-gating pattern. [claim:clm_inf05]
- Claimify (clm_001-004) produces decontextualized verifiable claims and discards ambiguous sentences; alignment/MiniCheck scoring (clm_081) and AlignScore (clm_028) consume text-pair inputs, so heterogeneous sources must first be normalized into comparable atomic claims, with Claimify's conservative discarding trading coverage for precision. [claim:clm_inf06]
- HALO (clm_020-025) is a low-cost, no-retraining half-life filter with per-category decay and a theta threshold, and clm_026/clm_027 show measurable accuracy gains from filtering, making it the cheap mechanism for RF's stale-claim flag. [claim:clm_inf09]
- High precision/low recall (clm_070) makes a flag trustworthy enough to auto-record, but the difficulty hierarchy (clm_068: self/conditional from 0.006-0.456) and 74% human agreement (clm_065) mean low-accuracy classes and single-signal flags must be human-adjudicated. [claim:clm_inf12]
- The evidence-extraction work (clm_035,038) argues for localizing the inconsistent sentence set, RnR+CF is the best method (clm_041), and ContraDocPaired adds size-two evidence sets (clm_039), so the log should record the specific conflicting claim/evidence pair, not a bare boolean. [claim:clm_inf14]
- Fields derive from cited mechanisms: conflict_type from the Gokul taxonomy (clm_073), evidence localization from clm_038, valid_time/as_of from the temporal-corruption design (clm_017), detector_score/threshold from MiniCheck binary thresholding (clm_081) and triple-judge (clm_057), and resolution_status from the precision/recall auto-vs-human split (clm_070,044). [claim:clm_inf15]
- Inter-context conflict (clm_044) is defined over comparable contextual statements; GraphRAG already extracts structured claim attributes like dates/events (clm_049) and temporal corruption (clm_017) shows context fields gate validity, so adding scope/measurement qualifier fields and matching on them before comparison encodes the genuine-vs-scope distinction. [claim:clm_inf16]
- RF's contradiction_log is an inter-context problem (clm_044, clm_071); LLM claim-pair judging directly addresses contradictions among source documents, NLI/alignment scorers (clm_028,031,032,079) give a cheap support/refute signal, and KG methods (clm_008,057) need a constructed entity graph as a prerequisite, making them a later-stage option. [claim:clm_inf01]
- clm_066 gives the only measured detection accuracies, clm_070/clm_076 establish the high-precision/low-recall profile, and clm_065 shows even humans only reach 74% agreement, so a negative LLM result is uninformative about true consistency. [claim:clm_inf02]
- clm_007 shows discarding is rare (<=5.4% and 0.8%), and clm_002/clm_003 establish the conservative interpretation policy, so adopting Claimify-style extraction loses little corpus while gaining interpretation reliability. [claim:clm_inf07]
- When-Facts-Expire corrupts only temporal context while keeping S/P/O fixed (clm_017) precisely because static KGs that ignore time produce false contradictions (clm_013); this proves temporal context is a distinct axis that must be checked before flagging a contradiction (clm_014). [claim:clm_inf08]
- Temporal-KG work (clm_013,017) and inter-context contradiction work (clm_044,071) target different questions; the static-KG false-inference problem (clm_013) is exactly what arises when temporal validity is folded into contradiction detection, so RF should keep them as separate flags. [claim:clm_inf10]
- GraphJudge defines triple classification (clm_057), filters meaningless/unsupported triples (clm_062), and shows a fine-tuned 7B judge reaches >90% while un-tuned BERT/GPT-4o fail (clm_063,059), so orphan/unsupported-claim flagging benefits from a tuned judge rather than zero-shot prompting. [claim:clm_inf11]
- clm_067/clm_074 show CoT is architecture-dependent (+31%/+46% Claude, -26% Llama-70B) and clm_075 shows CoT even hurt type-detection broadly, so the prompt strategy must be conditioned on the arbiter model family. [claim:clm_inf13]
...
- Prompting strategy in RF's arbitration step must be model-conditioned: chain-of-thought raised Claude-3 Sonnet detection by 31% (Haiku 46%) but degraded Llama-70B by 26%, so RF should enable CoT for Claude-family arbiters and disable it for Llama-family arbiters rather than applying a uniform prompt template. [claim:clm_inf13]
