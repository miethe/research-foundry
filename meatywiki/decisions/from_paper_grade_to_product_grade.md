---
id: mwb_20260622_dr_from_paper_grade_to_product
evidence_bundle_id: bundle_20260615_intent_research_20260614_what_evidence_grounded
target_page: meatywiki/decisions/from_paper_grade_to_product_grade.md
writeback_type: decision_record
status: written
summary: 'Decision record from run rf_run_20260614_what_evidence_grounded_acceptance_test_suites: The
  coupling theorem (clm_024) is a proven sufficiency condition decidable by the transducer''s degree test
  (clm_026); it'
key_claims:
- claim_id: clm_inf02
  include: true
...
# Decision Record: From Paper-Grade to Product-Grade: Acceptance-Test Suites and Round-Trip Validation Protocols for Crochet Patterns and Meshes
...
AmiGo's coupling theorem (Observation 2.3) is the single strongest literature-grounded crochetability acceptance criterion available: it gives a decidable, per-row-pair predicate (consecutive rows coupled => valid sc/inc/dec instructions exist) that is constructively checkable via the finite-state transducer, making it the recommended formal backbone of the inverse-engine acceptance gate rather than heuristic shaping-rate caps alone. [claim:clm_inf02]

## Rationale

- The coupling theorem (clm_024) is a proven sufficiency condition decidable by the transducer's degree test (clm_026); it strictly subsumes the KnitWit plan's looser 'no impossible transitions / bounded inc-dec rate' guarantee (clm_019), so it should anchor the acceptance gate. [claim:clm_inf02]
- CSR via the CrochetPARADE validator is execution-grounded (clm_034, clm_036), and the documented sharp decay from BLEU/ROUGE to compilation (clm_038) is direct evidence that surface/model-judge scoring is unreliable, so deterministic checks must backstop it. [claim:clm_inf03]
- AmiGo generates patterns from meshes (clm_039), the force-directed method reconstructs 3D from patterns (clm_045), the KnitWit plan defines the corpus and complexity metrics (clm_020), and coupling supplies the crochetability gate (clm_024) - together a closed forward-and-back protocol. [claim:clm_inf06]
- Tension flagging (clm_012, clm_036), topological/stitch-count correctness and shape-match-within-tolerance (clm_017, clm_018) are exactly the discrete machine signals that bucket into green/yellow/red, giving a deterministic trust meter. [claim:clm_inf08]
- Coupling and CSR are executable proofs of crochetability acceptance (clm_024, clm_034); shape fidelity is only shown on simple forms (clm_056) and CrochetPARADE's poor scaling to tens of thousands of stitches (clm_014) flags an unproven performance/mobile gap, separating the two readiness levels. [claim:clm_inf11]
- CrochetPARADE one-by-one attachment to consecutive previous stitches (clm_010) and its mismatched-count/impossible-attachment checks (clm_036), AmiGo's sc/inc/dec alphabet and transducer degree rule (clm_024, clm_026), and the KnitWit plan's mandated IR rules (clm_016) collectively specify the same four invariant families, which compose into one suite. [claim:clm_inf01]
- The KnitWit plan names Hausdorff/volumetric distance plus topological correctness (clm_017); Greer-Mould report shape/size accuracy on simple forms with a per-stitch node/edge graph (clm_056, clm_041), justifying the combined distance+topology metric set. [claim:clm_inf04]
- The plan names the distance metrics and a 20-50 mesh scoring set (clm_017, clm_020) but cites no numeric thresholds; the force-directed work's unit-edge limitation and trial-and-error tuning (clm_057) confirm geometry is uncalibrated, so tolerance bands are unestablished in the literature. [claim:clm_inf05]
- CrochetBench's task ladder, CSR, error taxonomy, and validator (clm_033-clm_036) are instruction-level acceptance tools with no mesh or geometric-distance component, so they cover the IR-validity half but leave the shape-fidelity half unfilled. [claim:clm_inf07]
- AmiGo's column/row-edge generator graph (clm_041) and Greer-Mould's horizontal/vertical/constraint-edge renderer graph with increase-only multi-node encoding (clm_048, clm_053) are different graph dialects; the IR's rounds/ops schema (clm_016) is the neutral layer that reconciles them. [claim:clm_inf09]
- Greer-Mould flag constant unit edge length as the primary accuracy limit and propose gauge swatches (clm_057), AmiGo uses a single fixed stitch width (clm_023), and the KnitWit plan names gauge/tension as a top risk (clm_022), so uncalibrated geometry threatens shape-distance validity. [claim:clm_inf10]
- The KnitWit plan defines the two flagship capabilities and their acceptance criteria - the 3D visualizer (clm_017), the inverse pattern generator (clm_018, clm_020), and the Crochet IR validation rules (clm_016) - and the synthesis (clm_inf01) traces those acceptance signals to the amigurumi-generation/pattern-representation literature; this sentence is the scope framing over that mapping. [claim:clm_inf12]

## Consequences

- Acceptance scoring should be tiered: deterministic execution-grounded checks (Compilation Success Rate via a CrochetPARADE-style validator, stitch-count/attachment consistency, coupling) must be the hard pass/fail gate, while any LLM/VLM judge is advisory only, because CrochetBench shows model performance collapses moving from surface metrics (BLEU/ROUGE) to structural compilation - i.e., string-similarity scoring systematically overstates correctness for spatial-instruction tasks. [claim:clm_inf03]
- A reproducible mesh->pattern->preview round-trip benchmark is constructible by composing existing pieces: use AmiGo's mesh->Crochet-Graph->pattern generator as the forward leg and a force-directed-graph-layout reconstruction as the preview leg, scoring the loop on (a) crochetability via the coupling/CSR gate, (b) round-trip visual/shape similarity, and (c) pattern-complexity metrics (max inc/dec rate per round, total rounds, stitch-count variance) over the sphere/egg/pear/animal-primitive corpus. [claim:clm_inf06]
- The acceptance signals map cleanly onto a tiered user-facing confidence meter: GREEN = coupling/CSR pass AND all per-round stitch counts validate AND shape distance within band; YELLOW = pattern compiles and closes but shape distance is marginal OR the validator flags over/under-stretched (tension) stitches; RED = any unresolvable attachment, impossible stitch-count transition, or compilation failure; each tier is backed by a deterministic machine check, so the meter never depends on a model judge alone. [claim:clm_inf08]
