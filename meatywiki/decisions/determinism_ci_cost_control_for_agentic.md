---
id: mwb_20260622_dr_determinism_ci_cost_control_for
evidence_bundle_id: bundle_20260615_intent_research_20260614_determinism_ci_cost
target_page: meatywiki/decisions/determinism_ci_cost_control_for_agentic.md
writeback_type: decision_record
status: written
summary: 'Decision record from run rf_run_20260614_determinism_ci_cost_control_for_agentic: FakeListLLM
  returns scripted responses in order and pytest-recording defaults to ''none'' (no network); both are
  fully det'
key_claims:
- claim_id: clm_inf05
  include: true
...
# Decision Record: Determinism & CI cost-control for agentic eval harnesses
...
For RF's swarm harness, the most reliable CI-gating determinism strategy is to stub the model leg entirely (FakeListLLM scripted responses or recorded VCR/pytest-recording cassettes) rather than to chase API-level reproducibility, because stubs and cassettes are byte-deterministic and network-free by construction whereas seed/temperature only reduce, not remove, live variance. [claim:clm_inf05]

## Rationale

- FakeListLLM returns scripted responses in order and pytest-recording defaults to 'none' (no network); both are fully deterministic, unlike best-effort seed/temperature, so stubbing is the more reliable gate. [claim:clm_inf05]
- The Completion Function Protocol generalizes tool-using-agent I/O, and VCR cassettes persist each HTTP request/response in ordered YAML; asserting on that ordered request log captures the tool trajectory without depending on output prose. [claim:clm_inf06]
- Cached/stubbed requests give near-free deterministic reruns suitable for per-PR gating; live model-graded judgment is inherently variable and best-effort, so it belongs on a non-blocking nightly leg. [claim:clm_inf07]
- Checked-in caches replay on later runs at near-zero cost, and host-scoped caching targets exactly the priced model endpoints, maximizing spend reduction while preserving live signal elsewhere. [claim:clm_inf08]
- A per-branch concurrency group with cancel-in-progress limits concurrent runs to one and kills superseded runs, directly bounding live-API spend triggered by frequent pushes. [claim:clm_inf09]
- Caching addresses spend and xdist addresses latency; LangSmith's marker is xdist-compatible, so the two can be stacked on the same deterministic leg. [claim:clm_inf10]
- Model-graded scores are variable and LangSmith supports comparing results over time; a delta-vs-baseline gate absorbs judge noise while still catching genuine regressions, unlike a brittle absolute pass mark. [claim:clm_inf11]
- Phoenix experiments hold inputs and criteria fixed across versions so improvement/regression is measured not assumed, which is exactly what a score-delta gate needs to attribute change to code. [claim:clm_inf12]
- 'none' mode prevents live calls so drift surfaces as a failed replay; the 'rewrite' mode regenerates cassettes from scratch, and system_fingerprint drift is a concrete upstream-change trigger for refresh. [claim:clm_inf13]
- system_fingerprint changes flag backend config drift for OpenAI; Anthropic exposes no equivalent field, so Claude-backed cassettes need a calendar-based refresh instead of a drift signal. [claim:clm_inf14]
- Because stubbed/cached legs are fully deterministic, a failure there is signal not noise, so retries belong only on the inherently variable live leg to avoid masking regressions. [claim:clm_inf17]
- Deterministic templates assume low output variation, but both vendor APIs disclaim reproducibility, so text-equality gates will flake; the eval must assert on tool-call structure instead. [claim:clm_inf01]
- OpenAI exposes seed and system_fingerprint; Anthropic exposes neither; only batch-invariant kernels produced fully identical outputs, ranking the three tiers. [claim:clm_inf02]
- Batch-invariant kernels gave full determinism but only on self-hosted dense models at TP1/TP2; hosted Claude/OpenAI users cannot reach into the serving stack, so this strategy is infra-gated. [claim:clm_inf03]
- Reported slowdowns of 34.35%/61.5%/1.6-2.1x quantify the determinism tax; that overhead is acceptable for batch/offline runs but not for fast interactive PR gating. [claim:clm_inf04]
- The Messages API documents only temperature/top_p/top_k, warns temp 0 is non-deterministic, and exposes no seed, so any determinism for a Claude-backed swarm must be built in the harness, not requested from the API. [claim:clm_inf15]
- Cassettes are near-free and deterministic with full request fidelity; batch-invariant kernels are fully deterministic but carry a 34-62% tax; live seed remains best-effort, yielding the three-way ranking across the matrix axes. [claim:clm_inf16]
- The memo's scope-and-ranking thesis is an analytic synthesis over the surveyed strategies (Anthropic exposes no seed clm_025; cassette/stub legs are byte-deterministic clm_028/clm_047; committed caches give near-free deterministic reruns clm_075; only batch-invariant kernels reach bit-identical output clm_070), not a single sourced fact. [claim:clm_inf00]
- Basic templates assume little output variation (clm_004) but live determinism is only best-effort (clm_044/clm_045), so text-equality flakes; model-graded evals (clm_005) delegate to a model whose judgment varies run to run, so neither template extreme is a stable gate for a tool-using swarm. [claim:clm_inf18]

## Consequences

- The recommended trajectory-assertion pattern is to wrap the agent in OpenAI evals' Completion Function Protocol (which standardizes tool-using-agent I/O) and assert on the recorded tool-call sequence and arguments captured by a VCR-style cassette, treating the cassette's ordered request log as the canonical trajectory rather than asserting on final answer text. [claim:clm_inf06]
...
- The cached-replay leg and the pytest-xdist parallelism flag are complementary rather than competing cost levers: caching removes per-run API spend while -n auto removes wall-clock time, so an RF deterministic leg should combine committed cassettes with pytest -n auto to keep the gate both cheap and fast. [claim:clm_inf10]
...
- Mapping onto Claude Agent SDK primitives, the load-bearing gap for RF's swarm harness is the absence of any server-side seed or reproducibility parameter on the Anthropic Messages API: RF cannot gate on Claude output determinism at all and must implement trajectory determinism entirely client-side via stubbing, cassette replay, and tool-sequence assertions. [claim:clm_inf15]
- On the comparison axes of stability, spend, and tool-call-signal preservation, recorded/cassette replay dominates for per-PR gating (high stability, near-zero spend, full tool-sequence fidelity), batch-invariant inference dominates for offline reproducibility (highest stability, high spend, full fidelity), and live seed/temperature scores lowest on stability for hosted Claude/OpenAI while being the only option that exercises real current model behavior. [claim:clm_inf16]
- Synthesizing the determinism strategies and CI cost-control patterns available to an agentic evaluation harness, they can be ranked for Research Foundry's swarm-of-agents use case, with client-side stubbing/cassette replay dominating per-PR gating and self-hosted batch-invariant inference dominating offline reproducibility. [claim:clm_inf00]
- Neither extreme of the eval-template taxonomy fits a tool-using swarm well, because output-text equality is too brittle against best-effort live determinism while a model-graded judge is itself variable run to run. [claim:clm_inf18]
