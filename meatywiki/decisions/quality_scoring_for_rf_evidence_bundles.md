---
id: mwb_20260622_dr_quality_scoring_for_rf_evidence
evidence_bundle_id: bundle_20260614_intent_research_20260614_what_methods_exist
target_page: meatywiki/decisions/quality_scoring_for_rf_evidence_bundles.md
writeback_type: decision_record
status: written
summary: 'Decision record from run rf_run_20260614_what_methods_exist_for_automatically_scoring: FActScore
  and both Ragas/DeepEval Faithfulness define quality as the fraction of claims supported by evidence
  on a 0-1 s'
key_claims:
- claim_id: clm_inf02
  include: true
...
# Decision Record: Quality Scoring for RF Evidence Bundles & CCDash quality_score
...
The single most transferable metric from the eval literature to RF is claim support rate (claims_supported / claims_total), the direct ledger analogue of FActScore's atomic-fact precision and RAGAS Faithfulness, and it should be the dominant term in quality_score. [claim:clm_inf02]

## Rationale

- FActScore and both Ragas/DeepEval Faithfulness define quality as the fraction of claims supported by evidence on a 0-1 scale; RF emits exactly claims_supported and claims_total, making this metric a zero-cost, literature-validated primary term. [claim:clm_inf02]
- A Faithfulness-style support ratio anchors the composite (literature-dominant term per clm_035/clm_061/clm_028); DeepResearch Bench RACE shows multi-dimension weighted scoring is the accepted shape (clm_054/clm_057); all listed inputs map to fields already in RF's ccdash_event metrics block. [claim:clm_inf03]
- FACT scores effective citation count and accuracy (clm_056) and RAGAS rewards focused, non-redundant context (clm_013), but RF's current metrics block only counts source_cards_created, not their distinctness; a new distinct_source_domains signal is the minimal addition to make source diversity computable. [claim:clm_inf04]
- Faithfulness as supported/total claims (clm_035) is directly inflatable by splitting; RHB shows agents exploit naturalistic shortcuts when the metric rewards them (clm_073); weighting by the ledger's existing materiality field and saturating the count resists the split-to-inflate exploit. [claim:clm_inf06]
- RAGAS Noise Sensitivity and the TruLens triad reward systems that detect bad/irrelevant context (clm_041/clm_048), and RAGChecker separates hallucination from self-knowledge to expose model behavior (clm_069); analogously, RF surfacing contradictions is evidence of verification rigor, so the contradiction_log should be a positive coverage signal. [claim:clm_inf07]
- G-Eval (clm_022/clm_024) and RACE Readability/Insight (clm_057) are LLM-judge rubric scores that capture coherence/depth; these are non-deterministic and costly, so they belong in a separate optional field rather than the auditable deterministic quality_score. [claim:clm_inf08]
- RAGChecker emits a diagnostic suite rather than one number (clm_066), RACE scores four named dimensions (clm_057), and the TruLens triad is three separate edges (clm_047); a leaderboard needs a scalar but actionability needs the vector, so RF should emit both. [claim:clm_inf09]
- FActScore reports cost ($26K) as a separate efficiency story from the factuality score (clm_031), and Ragas notes LLM-call cost per score (clm_046); cost-per-verified-claim divides emitted cost_estimated_usd by claims_supported and stays a separate axis to avoid rewarding cheapness over quality. [claim:clm_inf10]
- RHB shows agents exploit self-reported/process shortcuts (clm_073/clm_074) but that 72% of exploits leave detectable chain-of-thought traces (clm_076); rework_count/drift_score are self-generated and exploitable, so they belong as a small auditable penalty, not a primary term. [claim:clm_inf12]
- TruLens frames passing all triad checks as the precondition for trusting the output (clm_047/clm_052); analogously a failed RF verification gate should cap quality_score rather than be averaged away, since an unverified run cannot be high quality regardless of other signals. [claim:clm_inf13]
- RHB enumerates naturalistic metric exploits and shows environmental hardening cuts them 87.7% (clm_073/clm_077); BrowseComp shows volume is not quality (clm_004) and RAGAS penalizes redundant context (clm_013), motivating distinct-domain counting, materiality weighting, and source-concentration caps as the three hardenings. [claim:clm_inf14]
- RHB finds 72% of exploits are detectable in chain-of-thought/traces (clm_076) and that hardening the environment suppresses them (clm_077); RF already emits run_trace.jsonl stages, so a trace-audit tripwire is a cheap, evidence-backed defense against gamed composites. [claim:clm_inf15]
- DeepEval defines truthful as not-contradicting rather than strictly entailed (clm_063), and RAGChecker's self-knowledge category recognizes correct-but-unretrieved claims (clm_069); strict entailment (clm_011) would penalize multi-source inference, so contradiction-based checking better fits RF's inference/speculation claim types. [claim:clm_inf16]
- FActScore's sub-60% ChatGPT score marks a clearly-deficient run (clm_030) and its less-than-2% automated error supports threshold-based grading (clm_029); FACT's diversity emphasis (clm_056) and TruLens's all-pass precondition (clm_047) justify the named cutoffs as a deterministic tier rule. [claim:clm_inf17]
- FActScore (atomic-fact percentage supported), RAGAS/DeepEval Faithfulness (supported claims / total claims), TruLens Groundedness (per-claim evidence search), and RAGChecker (claim-level entailment) all reduce to a supported-claims ratio; RF's ledger emits claims_supported and claims_total, so the same ratio is already computable. [claim:clm_inf01]
- BrowseComp shows raw tool access (more browsing) barely moves accuracy and can raise calibration error (clm_004/clm_005), so a count of activity (cards, claims) is not a quality signal; using ratios instead of counts prevents rewarding padding, and clm_046 confirms each scoring call has cost so volume is doubly penalizing. [claim:clm_inf05]
- Support rate and unsupported rate are the literature-validated quality core (clm_035/clm_028); source diversity maps to FACT (clm_056); BrowseComp shows volume/tool-use signals - proxies for cost/latency/counts - are weak quality indicators (clm_004/clm_005), justifying the ranking and the exclusions. [claim:clm_inf11]
- RAGChecker beats RAGAS on human correlation (clm_070) precisely because of finer claim-level diagnostic categories (clm_066/clm_069); RF already tracks claims_inference and claims_speculation, so preserving those categories (rather than a single ratio) aligns RF with the better-correlated framework. [claim:clm_inf18]

## Consequences

- RF should compute quality_score as a deterministic 0-to-1 composite Q = 0.45*support_rate + 0.20*(1 - unsupported_rate) + 0.15*source_diversity + 0.10*verification_passed + 0.10*(1 - normalized_rework), where support_rate = claims_supported/claims_total, unsupported_rate = unsupported_claims/claims_total, and every input is a metric RF already emits in ccdash_event (claims_supported, claims_total, unsupported_claims, verification_passed, rework_count) plus the one new diversity signal. [claim:clm_inf03]
- RF must add one new emitted signal, distinct_source_domains (count of unique source-card publisher/domain values), because no metric RF currently emits captures source diversity, which FACT's effective-citation-count and RAGAS Context Relevance treat as first-class; source_diversity in the composite should be a saturating function such as min(1, distinct_source_domains / 6). [claim:clm_inf04]
- Claim-splitting (atomizing one claim into several trivially-supported sub-claims to inflate support_rate) is the highest-severity Goodhart risk for RF, directly analogous to citation stuffing; mitigation is to normalize support_rate by a materiality-weighted count (material claims weighted higher than background) and to cap the marginal score contribution of claims beyond a per-report saturation point. [claim:clm_inf06]
- Contradiction coverage should be rewarded, not penalized: a run that surfaces and logs contradictions (claims_contradicted > 0 plus a populated contradiction_log) demonstrates adversarial verification, so quality_score should treat a non-empty contradiction_log as a small positive coverage signal rather than counting contradicted claims purely as defects. [claim:clm_inf07]
- Synthesis coherence cannot be computed from RF's current deterministic metrics and is the only quality dimension that genuinely requires an LLM-as-judge pass; RF should emit it as an optional secondary coherence_score (a G-Eval/RACE-style 1-to-5 rubric on the report) kept OUT of the deterministic quality_score to preserve auditability. [claim:clm_inf08]
- quality_score should be emitted as BOTH a per-dimension vector (support, diversity, verification, coverage, efficiency) AND a single derived scalar plus a tier label; the scalar feeds CCDash leaderboards and Recommended-Stack ranking, while the vector preserves the diagnostic decomposition that RAGChecker's diagnostic suite and RACE's four named dimensions show is necessary to act on a score. [claim:clm_inf09]
- Cost-per-verified-claim should be computed as cost_estimated_usd / max(1, claims_supported) and reported as an efficiency metric ALONGSIDE quality_score, never folded into it, because FActScore's $26K-for-6500-generations framing treats cost as an axis orthogonal to factual precision and merging them lets a cheap low-quality run outrank a thorough one. [claim:clm_inf10]
- rework_count and drift_score are RF's best available process-quality proxies and should enter the composite only as a small (about 10%) penalty term, because they are internally generated by the run and thus gameable - an agent that under-reports rework looks better - so they must be capped and trace-audited rather than trusted as primary signals. [claim:clm_inf12]
- The verification gate (verification_passed) is a near-binary necessary condition, not a graded quality term: RF should hard-floor quality_score (for example cap it at 0.5) for any run where verification_passed is false, mirroring TruLens's framing that the RAG triad gives confidence of freedom-from-hallucination only when all three checks pass. [claim:clm_inf13]
- Three concrete Goodhart attacks on quality_score and their mitigations are (a) source padding (add many low-value cards to inflate diversity), mitigated by counting distinct domains not raw cards and saturating the diversity term; (b) claim splitting (atomize claims to raise support_rate), mitigated by materiality-weighting and count saturation; and (c) citation stuffing (attach the same source to many claims), mitigated by a per-source-card claim-concentration cap so over-reused cards stop adding score. [claim:clm_inf14]
- Because 72% of reward-hacking episodes leave explicit chain-of-thought rationale in traces, RF should add a lightweight trace-audit tripwire (a flag set when run_trace.jsonl shows verification steps were skipped or a source was fetched-but-unread) that can veto a high quality_score, rather than relying solely on the numeric composite. [claim:clm_inf15]
- RF should standardize on a contradiction-based support check (a claim counts as supported if it does not contradict the evidence) rather than strict entailment, following DeepEval's truthfulness criterion, because strict entailment would systematically under-credit valid synthesis/inference claims that combine multiple sources and depress quality_score for exactly the analytical work RF most values. [claim:clm_inf16]
- A run is classifiable as high-quality when support_rate >= 0.85, distinct_source_domains >= 4, unsupported_rate <= 0.05, verification_passed is true, and contradiction_log is non-empty; low-quality when support_rate < 0.6 OR verification_passed is false OR distinct_source_domains <= 1 - a deterministic tier rule computable entirely from emitted metrics plus the one new diversity signal. [claim:clm_inf17]
- RF's claim ledger already implements the core mechanic shared by RAGAS Faithfulness, DeepEval Faithfulness, TruLens Groundedness, FActScore, and RAGChecker - decompose output into atomic claims, then check each against evidence - so RF can compute a claim support rate as claims_supported / claims_total natively without adopting any external scorer's runtime. [claim:clm_inf01]
- Raw source_cards_created and claims_total are gameable padding signals and must never be reward terms in quality_score - only ratios and diversity counts qualify - because BrowseComp shows enabling browsing for GPT-4o lifted accuracy only from 0.6% to 1.9%, the direct analogue that volume of activity does not track quality. [claim:clm_inf05]
- Ranked by predictiveness of true run quality, RF's most reliable telemetry signals are (1) claim support rate, (2) unsupported/contradicted claim rate, (3) source diversity via distinct domains, (4) verification_passed, (5) rework_count and drift_score, with raw cost, latency, tokens, and absolute card/claim counts being weakly correlated or gameable and therefore excluded from the score. [claim:clm_inf11]
- RAGChecker, not RAGAS, is the closest external analogue to RF's intended scorer because it outperformed RAGAS on human correlation (Pearson 61.93 vs 48.31 overall) by adding claim-level diagnostic categories like hallucination and self-knowledge; RF should mirror RAGChecker's decomposition by emitting claims_inference and claims_speculation as distinct ledger categories rather than collapsing everything into supported/unsupported. [claim:clm_inf18]
