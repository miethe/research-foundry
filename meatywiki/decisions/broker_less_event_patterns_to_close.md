---
id: mwb_20260622_dr_broker_less_event_patterns_to
evidence_bundle_id: bundle_20260614_intent_research_20260614_what_patterns_file
target_page: meatywiki/decisions/broker_less_event_patterns_to_close.md
writeback_type: decision_record
status: written
summary: 'Decision record from run rf_run_20260614_what_patterns_file_watch_hooks_post: post-commit/post-merge
  run once per discrete VCS operation (clm_013, clm_014), whereas inotify coalesces identical unrea'
key_claims:
- claim_id: clm_inf01
  include: true
- claim_id: clm_inf02
...
# Decision Record: Broker-less Event Patterns to Close Local-First Agentic OS Seams
...
Git post-* triggers (post-commit, post-merge) are the best fit for RF's research->vault seam because they fire exactly once per atomic commit/merge with no event coalescing or queue-overflow loss, unlike file-watch primitives which coalesce identical events and silently drop on queue overflow. [claim:clm_inf01]

## Rationale

- post-commit/post-merge run once per discrete VCS operation (clm_013, clm_014), whereas inotify coalesces identical unread events and drops on overflow (clm_065, clm_066, clm_067); since RF research output lands as committed source cards/reports, the commit boundary is the natural exactly-once trigger. [claim:clm_inf01]
- watchdog/chokidar give low-latency local notification (clm_039), but FSEvents reports at directory granularity and forces rescans on coalescing/drops (clm_049, clm_051, clm_052) and inotify drops on overflow (clm_066, clm_067); the vault->graph seam therefore needs a reconciliation scan as the authoritative recovery path. [claim:clm_inf02]
- SSE is explicitly one-way server-to-client (clm_001), so a downstream tool cannot use it to push a handoff to an upstream producer; its auto-reconnect and 204-stop semantics (clm_002, clm_037) suit a read-only observer fan-out such as telemetry->governance dashboards. [claim:clm_inf03]
- synthesizes clm_inf01-inf05: commit-boundary determinism favors git triggers for research->vault (clm_013), low-latency local detection favors file-watch for vault->graph (clm_039), one-way broadcast favors SSE for telemetry->governance (clm_001, clm_034), and lossy primitives (clm_066) motivate an append-only log for replay/reconciliation. [claim:clm_inf07]
- watchdog selects the native backend per OS (clm_039) and chokidar's atomic (100ms, clm_028) and awaitWriteFinish (2000ms, clm_029) options debounce in-progress writes so the graph ingests only stable files; watchexec's 50ms debounce (clm_044) is tuned for command re-runs, not write-stability, making the library watchers a better fit for ingestion. [claim:clm_inf10]
- SSE one-way and file-watch shared-FS assumptions break across hosts (clm_001, clm_069), and overflow rescans (clm_066) signal volume saturation; git post-receive already runs server-side once per updated ref on a shared remote (clm_015, clm_016), making it the minimal cross-host fan-out step before adopting a dedicated broker. [claim:clm_inf14]
- fanotify fd-delivery bypasses authorization checks and is a security risk under CAP_SYS_ADMIN (clm_058), but FAN_REPORT_FID/DIR_FID identify objects by handle rather than fd (clm_057); so the race-free whole-tree scaling option (clm_056) is only safe for a governed local-first OS when used in handle-reporting mode. [claim:clm_inf17]
- the Last-Event-ID header and the persisting last-event-ID buffer (clm_033, clm_034) let a reconnecting client tell the server its last seen id so the server can replay missed events; combined with auto-reconnect (clm_037) this is a spec-level at-least-once resume that file-watch and git hooks lack. [claim:clm_inf04]
- inotify coalescing/overflow loses or merges events (clm_065, clm_066), post-merge is skipped on conflict and can recur on later merges (clm_014), and SSE id-resume is at-least-once replay (clm_034); none deliver exactly-once, so idempotent consumers are mandatory. [claim:clm_inf05]
- inotify rename pairing is inherently racy and may drop IN_MOVED_TO (clm_070) and watchexec warns renames are not portably trackable and can split across debounce boundaries (clm_045); a git post-commit hook sees the post-rename tree state atomically (clm_013), sidestepping the race entirely. [claim:clm_inf06]
- kernel event queues drop on overflow (clm_066, clm_067) and SSE reconnect loses in-flight events not yet flushed (clm_002), so durability requires persistence; the SSE id-resume model (clm_034) demonstrates that a stored last-position cursor enables replay, which an append-only file log generalizes to survive restarts. [claim:clm_inf08]
- post-merge is explicitly not executed when the merge fails due to conflicts (clm_014), creating a missed-event hole; storing a last-processed commit SHA and reconciling against HEAD mirrors the SSE last-event-ID cursor pattern (clm_034) to recover the gap. [claim:clm_inf09]
- editor save patterns produce delete+re-add that chokidar only partly coalesces within 100ms (clm_028) and Vim's backup-swap suppresses on-modified entirely (clm_041), while amend/rebase re-fire post-commit (clm_013) and lefthook skip can only suppress by branch/state not by content (clm_059); hashing content/SHA makes duplicate handoffs idempotent. [claim:clm_inf11]
- FSEvents coalesces near-simultaneous parent/child events into one MustScanSubDirs event (clm_051) and inotify rename pairs are non-atomic and interleavable (clm_070), so FS arrival order is unreliable; git commit ordering and per-ref post-receive stdin lines (clm_015) provide a monotonic order to key on instead. [claim:clm_inf12]
- inotify is non-recursive needing a watch per subdirectory (clm_069) capped by max_user_watches (clm_068, default raise needed per clm_031), and kqueue uses one fd per file and does not scale to deeply nested trees (clm_040); the race-free escape is fanotify mount/filesystem marks (clm_056) or polling once the limit is hit. [claim:clm_inf13]
- git post-commit/post-merge fire deterministically per VCS op (clm_013, clm_014) but are fire-and-forget with no replay; pairing them with a cursor-replayable log (modeled on SSE id-resume, clm_034) compensates for the drop/skip gaps (clm_066) to reach at-least-once with idempotent consumers. [claim:clm_inf15]
- lefthook is a single dependency-free Go binary (clm_021) with parallel execution (clm_022) and rich skip controls (clm_059), while pre-commit silently skips file-less notification hooks unless always_run:true is set (clm_008); for notification-style RF triggers the lefthook footprint and explicit skip semantics reduce the silent-skip failure mode. [claim:clm_inf16]

## Consequences

- For RF's vault->graph seam, a file-watch hook (watchdog on Linux/macOS, chokidar on Node) is the most responsive pattern, but it MUST be paired with a directory-rescan reconciliation pass because every native backend (inotify, FSEvents) signals lossy overflow conditions (IN_Q_OVERFLOW, MustScanSubDirs/Dropped) that force a full rescan rather than per-file delivery. [claim:clm_inf02]
- SSE is unsuitable as the primary trigger for any RF write-path seam (research->vault, vault->graph) because it is a one-way server-to-client channel that cannot carry a tool-to-tool handoff push; SSE's correct role in RF is the telemetry->governance fan-out, broadcasting governance events to dashboards/observers. [claim:clm_inf03]
- The recommended pattern-vs-seam mapping for RF is: research->vault = git post-commit/post-merge trigger; vault->graph = file-watch (watchdog/chokidar) plus reconciliation rescan; telemetry->governance = SSE fan-out with Last-Event-ID resume; with lightweight event sourcing (an append-only event log file) as the cross-cutting reconciliation substrate for all three. [claim:clm_inf07]
- RF should select watchdog (Python) or chokidar (Node) over raw inotify/FSEvents and over watchexec for the vault->graph watcher, because both libraries already abstract per-OS backend selection and provide write-coalescing debounce (chokidar atomic 100ms / awaitWriteFinish 2000ms) that prevents partial-file ingestion into the graph. [claim:clm_inf10]
- A minimal message broker becomes justified for RF only when the seams cross a network or host boundary or exceed local kernel watch/queue limits; the concrete threshold criterion is when (a) producers and consumers no longer share a filesystem, or (b) event volume causes recurrent IN_Q_OVERFLOW/dropped-event rescans, at which point git post-receive on a shared remote is the lowest-cost escalation before a true broker. [claim:clm_inf14]
- The fanotify file-descriptor security caveat (events deliver fds with no per-receiver authorization check under CAP_SYS_ADMIN) means RF should NOT escalate to fanotify mount/filesystem-wide marks for scaling unless the watcher runs unprivileged via FAN_REPORT_FID handle-based reporting, keeping the telemetry->governance trust boundary intact. [claim:clm_inf17]
- SSE's last-event-ID resume protocol (Last-Event-ID header plus persistent id field) gives the telemetry->governance seam at-least-once delivery across reconnects and tool restarts, making it the only candidate pattern in this evidence set with a built-in, standardized offline-gap recovery mechanism. [claim:clm_inf04]
- No candidate pattern in the evidence base provides exactly-once delivery: file-watch coalesces and drops events, git post-* hooks can re-fire (e.g. amended commits, re-merges) and are skipped on failure, and SSE explicitly guarantees only at-least-once via id-resume; therefore every RF seam MUST make its consumer idempotent rather than rely on transport-level exactly-once. [claim:clm_inf05]
- File-watch is fundamentally unreliable for tracking renames/moves in any RF seam: inotify's IN_MOVED_FROM/IN_MOVED_TO pairing is racy and may be incomplete, watchexec cannot portably distinguish old-vs-new paths, and these races make rename a leading cause of missed or duplicated handoffs; git triggers avoid this because a rename is just content in the next commit's tree. [claim:clm_inf06]
- Lightweight event sourcing implemented as an append-only newline-delimited log on the local filesystem is the highest-durability broker-less pattern because, unlike file-watch (bounded kernel queues that drop) and unlike in-memory SSE streams, an on-disk log survives tool restarts and offline periods and supports deterministic replay from a stored cursor. [claim:clm_inf08]
