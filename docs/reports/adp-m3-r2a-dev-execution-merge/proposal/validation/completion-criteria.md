...
### The validation-scope rule (AC-1/2/3) — sibling of R3, enforced in `reviewer-gate`

R3 presupposes a green test and asks whether the green path is production's path. This rule asks the
**prior** question: *is the supporting test passing at all, and did the gate even run the file that
tests this behaviour?* A reviewer can establish perfect `path-equivalence` for a test and approve
while that test is red — so R3 cannot cover it. They are siblings at the same seam, not one rule.

The gate runs a **`Measure` phase before `Review`** (`reviewer-gate.js`; the delta engine is
`.claude/skills/dev-execution/hooks/validation-scope.sh` → `validation_scope.py`), and the reviewer
is bound to three things:

- **Test-selection scope is the resolved SCOPE, not `files_changed`.** For each symbol the diff
  changed, every test file that references that symbol by name is in scope — including files the diff
  never touched. (Reading scope stays the diff, for reading cost; only test selection widens.) A
  change is scoped per file; the tests for its behaviour are scoped per behaviour, so the diff is the
  wrong index into the suite.
- **Regressions are judged by the base→head DELTA, never the absolute red count.** A file 61-red at
  base and 61-red at head carries no signal; a file that gained one *new* failing node id carries
  all of it. A test that stopped being collected at head ran nowhere and its absence *lowers* the
  failure count — a collected-regression or a disappeared node id is a regression, never an
  improvement.
- **An AC backed only by a red/absent test is NOT met.** `ac_verdicts[].supporting_tests` records
  `{nodeid, status}` per criterion, using the *measured* status; the gate cross-checks every claimed
  status against the measurement (the measurement wins) and rewrites a `met: true` whose support is
  entirely non-passing to `met: false`.

Outcomes split deliberately, exactly as R3's do: an AC met on a red test is an **ordinary rejection**
(`defect_class: ac-backed-by-red-test` — the implementer makes the test pass or drops the AC), while a
**missing or failed measurement, or an approval standing over a measured regression, is a
gate-integrity failure** (re-dispatch, no fix cycle — what did not finish is the reviewer or the
measurement). A truncated or unmeasurable scope downgrades the affected criteria to `unverifiable`,
never met.

Why enforced rather than documented: skillmeat PR #299 (2026-08-09) was approved 4/4 ACs "met" over a
suite containing a 3-red *untouched* file that asserted the exact behaviour the change removed, plus a
~61-red baseline file where new failures were invisible. The gate scoped validation to the files the
executor touched; no reviewer diligence within that scope could have found either defect. The prose
version of this rule then failed a second time inside this very feature — its own Phase 0 and Phase 1
were approved by gates that scoped to the touched files, while the whole launchpad suite was
collection-interrupted (0 tests running). Tracker `node_01KZKSJKK4W3WA80MEKF2GMNPM`.

> **Coverage boundary (honest):** the enforcement lives in `reviewer-gate.js` today.
> `execute-plan.js` and `execute-contract.js` land verdicts at `enforceEvidenceRules` and are **not
> yet** retrofitted — tracked as `node_01KZP97XJKQJ3K7JRPPWXFDB1E`, with a placeholder test in
> `tests/test_workflow_gate_integrity.py` that fails the moment the retrofit lands. Until then, a
> plan- or contract-driven gate still scopes test selection to `files_changed`.
