...
Build the SPIKE ADR-002 boundary: subprocess-per-agent-job spawn (FR-11), temp-file credential delivery (FR-12), write-time redaction firewall (FR-13, extending `governance.py`'s `scan_secrets`/`_redact` from a post-hoc scan to a write-time guard), and salted-HMAC key fingerprinting (FR-14). FU-1 (spawn-latency benchmark) runs in parallel, non-blocking, per D3 — it does not gate this phase's exit.
...
| SEC-2.2 | Credential temp-file delivery + crash-safe cleanup | Job-scoped temp file, mode `0600`, unique per-job path, unlinked immediately after the child reads it (FR-12) — never an env var. Crash-safe cleanup: kill the subprocess mid-run, assert the file does not survive; consider a reaper/janitor pass on job-store startup as defense-in-depth. | AC-5.2 (crash-safety test) and AC-5.3 (code-path audit: no `os.environ`/`subprocess env=` inheritance) both pass. | 1.5 pt | python-backend-engineer | sonnet | extended | SEC-2.1 |
| SEC-2.3 | Write-time redaction firewall | Extend `governance.py`'s `scan_secrets`/`_redact` into a guard applied to artifact + event payloads *before* persistence or streaming (FR-13) — not only the existing post-hoc scan. | AC-2.2 (every persisted/streamed event and artifact passed the guard) and AC-5.1 (secret-scan test, 0 raw matches) pass. | 1 pt | python-backend-engineer, backend-architect | sonnet | extended | JOB-1.3 |
...
- [ ] Code-path audit confirms no env-var/`subprocess env=` credential inheritance (AC-5.3).
