...
```jsonc
{
  "schema_version": "1.5",            // export contract version (this doc)
...
```jsonc
{
  ...
  "persistent_references": {           // optional; omitted by legacy ledgers
    "source_edition_id": "sed_...",
    "passage_id": "psg_...",
    "source_assertion_id": "ast_...",
    "assertion_version": 2,
    "canonical_claim_id": null,
    "canonical_claim_version": null,
    "inference_id": null
  },
  "sources": [ /* §6.1 */ ]
}
...
### 6.2 `persistent_references` — Optional Assertion Ledger Links (v1.5)

Schema 1.5 reserves durable links for a later assertion materialization phase:
`source_edition_id`, `passage_id`, `source_assertion_id`, `assertion_version`,
and optional canonical-claim or inference IDs. The block is emitted only when
the input claim ledger explicitly has it. Legacy ledgers omit it, and the export
service never invents IDs. This has no effect on source resolution, redaction,
or the schema 1.4 AOS UUID/native-alias behavior.
...
Schema 1.5 is **fully backwards compatible** with all prior schema versions (1.0 through 1.4):

- All fields introduced in 1.2 through 1.5 are **optional/nullable**.
...
- Schema `1.5`: adds optional `claims[].persistent_references`; omission retains legacy run-local claim semantics.
...
Schema version is automatically set to `"1.5"` in all exports produced by the current
...
| 2.0 | 2026-08-05 | **Clinical attestation marker (clearance-gates-v1 M4)** — added optional per-claim `clinical_attestation_status: "unattested"`, present only when `claim_clinical_eligibility()` (unchanged) finds the claim clinically eligible. Never derived from `clearance.blocked_scopes`. Key omitted (not `null`) in the non-clinical case. Additive; the 7 committed `pediatric_cds` bundles under `runs/` still validate unchanged. (Intermediate 1.5–1.9 bumps between this row and 1.4 above are documented in `rf-run-export-schema.json`'s own `schema_version` description, not backfilled into this table.) |
