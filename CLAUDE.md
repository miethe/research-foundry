...
External models are available as **opt-in** supplements. Claude Opus remains the sole orchestrator.
...
| Debug escalation | Codex | After 2+ failed Claude cycles |
...
├── .claude/                # Agent configuration, skills, rules, commands
...
- `.claude/rules/delegation-modes.md` — Five delegation modes (A–E) calibrating agent autonomy
...
- `.claude/context/key-context/testing-patterns.md` — Test structure and patterns
...
- **Scopes**: `user` scope (`~/.claude/`) is global; `local` scope (`./.claude/`) is per-project.
