...
def test_claim_counts_and_top_level_shape(tmp_foundry: FoundryPaths) -> None:
    build_run(tmp_foundry)
    data = svc.export_run(tmp_foundry, "rf_run_test001")
    assert data["schema_version"] == svc.EXPORT_SCHEMA_VERSION == "1.9"
...
def test_schema_version_bumped_to_1_7(tmp_foundry: FoundryPaths) -> None:
    ...
    assert svc.EXPORT_SCHEMA_VERSION == "1.9"
...
def test_schema_12_run_json_loads_without_context_key(tmp_foundry: FoundryPaths) -> None:
    ...
    The export service always writes the current schema version now (1.5); this test ensures that a consumer
...
def test_context_full_pipeline_round_trip(tmp_foundry: FoundryPaths) -> None:
    """BE-004: full export pipeline round-trip → context present, schema 1.7.
