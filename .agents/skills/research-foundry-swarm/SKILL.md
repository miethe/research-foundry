...
**Adapter degradation:** All five adapters (`gpt_researcher`, `paperqa2`, `claude_agent_sdk`, `opencode`, `litellm`) degrade to deterministic stubs unless their extra is installed AND opt-in real mode is enabled. Real mode for `claude_agent_sdk` targets the Codex Agent SDK Python package, not the Codex CLI harness. Stubs are safe and sufficient for testing the pipeline shape.

### Path B — Codex-orchestrated swarm (works today, recommended)

This is the clean integration pattern: the Codex agent (or a multi-agent workflow) performs discovery using its own tools and subagents, then feeds findings into the RF run as source cards. RF stays the governance + claim-ledger + verifier spine; the swarm is disposable discovery muscle.
...
3. **Agent-driven discovery:** The Codex agent (or subagents it spawns) uses web search, document reads, API calls, or any available tool to locate sources. Each located source is a candidate to feed in.
...
**Key architectural fact:** The Codex multi-agent workflow is the outer orchestrator. `rf` is never the caller of Codex — it is the governance and evidence spine that the orchestrator feeds into.
...
- **Per-command reference + 21-step loop**: `.Codex/skills/research-foundry/SKILL.md`
...
- **Author artifacts for MeatyWiki ingestion**: `.Codex/skills/meatywiki-author/SKILL.md`
- **Drive the MeatyWiki CLI (ingest → compile → lint)**: `.Codex/skills/meatywiki/SKILL.md` — full lifecycle incl. Portal API/MCP: `.Codex/skills/meatywiki-suite/SKILL.md` (require the MeatyWiki app from `../meatywiki`)
- **Agent Review Council (offline gate over report + ledger)**: `.Codex/skills/council-review/SKILL.md` — backs `rf council` (exit code 7 path, step 16 of the 21-step loop); vendored reviewer agents live in `.Codex/agents/council/` (`council-coordinator.md`, `architecture-reviewer.md`, `correctness-reviewer.md`, `domain-research-reviewer.md`, and the full roster)
...
- **intenttree-cli**: `.Codex/skills/intenttree-cli/SKILL.md` — live IntentTree server interface for intent/tree management; note that `rf intent` and `rf tree` (used in step 2 of this skill's Path B) are the offline RF-native subset of this capability; load `intenttree-cli` only when a live IntentTree server is available and required
- **Workflow authoring for RF swarm/council scripts**: `.Codex/skills/workflow-authoring/SKILL.md` — use when authoring or extending `.Codex/workflows/research-foundry-swarm.js` or `.Codex/workflows/research-foundry-council.js`
- **research-foundry-swarm Workflow script**: `.Codex/workflows/research-foundry-swarm.js` — Codex-orchestrated Path B discovery swarm; registered in `.Codex/specs/workflows/workflow-registry.md`
- **research-foundry-council Workflow script**: `.Codex/workflows/research-foundry-council.js` — offline council gate over a run's report and claim ledger; registered in `.Codex/specs/workflows/workflow-registry.md`
- **RF discovery swarm agents**: `rf_discovery_lead` (swarm orchestrator), `rf_deep_reader` (full-text extraction), `rf_domain_researcher` (domain-specific source location) — used by `research-foundry-swarm.js`; definitions under `.Codex/agents/research/`
