...
| Artifact | RF path |
|---|---|
| Master contract | `/Users/miethe/dev/homelab/development/research-foundry/.Codex/specs/workflows/workflow-authoring-spec.md` |
| Schemas directory | `/Users/miethe/dev/homelab/development/research-foundry/.Codex/specs/workflows/schemas/` |
| Workflow registry | `/Users/miethe/dev/homelab/development/research-foundry/.Codex/specs/workflows/workflow-registry.md` |
| Pattern library | `/Users/miethe/dev/homelab/development/research-foundry/.Codex/skills/dev-execution/orchestration/workflow-patterns.md` |
| Workflow scripts output | `/Users/miethe/dev/homelab/development/research-foundry/.Codex/workflows/` |
...
Both scripts live (or will live) under `.Codex/workflows/` and follow the same five-step
...
See also: `.Codex/skills/research-foundry/SKILL.md` for the RF research run lifecycle that
calls these workflows, and `.Codex/skills/research-foundry-swarm/SKILL.md` for swarm-specific
orchestration context.

---

Governed procedure for authoring SkillMeat Dynamic Workflow scripts. Operationalizes
`.Codex/specs/workflows/workflow-authoring-spec.md` §5.4 into a repeatable five-step flow.
...
1. **Master contract** — `.Codex/specs/workflows/workflow-authoring-spec.md`
...
2. **Per-workflow spec** — `.Codex/specs/workflows/<name>-workflow-spec.md`
...
3. **Registry entry** — `.Codex/specs/workflows/workflow-registry.md`
...
Consult `.Codex/skills/dev-execution/orchestration/workflow-patterns.md`.
...
**File**: `.Codex/workflows/<name>.js` (plain JS, not TypeScript)
...
- **Provider routing**: workflow scripts dispatch `agent(prompt, { agentType })` only. They NEVER call the `delegation-router` resolver at runtime. The resolver runs at Opus pre-flight; the resolved `RoutingRecord.agent_type_id` becomes the `agentType` value passed to `agent()`. See `.Codex/specs/provider-routing-spec.md` for routing rules.
...
Full schemas: `.Codex/specs/workflows/schemas/execution-graph.schema.json` and
`.Codex/specs/workflows/schemas/execution-report.schema.json`.
...
node --check <(
  sed 's/^export const meta/const meta/' .Codex/workflows/<name>.js \
...
automatically for any `.Codex/workflows/*.js` file.
...
**Save**: The script is already at `.Codex/workflows/<name>.js`. No additional save step needed —
the file IS the saved workflow. Reference via `/<name>` command or `workflow('<name>', args)`.

**Register**: Add or update the entry in `.Codex/specs/workflows/workflow-registry.md`:

```
| `<name>` | `.Codex/specs/workflows/<name>-workflow-spec.md` | `.Codex/workflows/<name>.js` | `active` | nick | P<N> | <one-line description> |
```
...
| `.Codex/specs/workflows/workflow-authoring-spec.md` | Master contract (authoritative) |
| `.Codex/specs/workflows/workflow-registry.md` | Registry of all workflows + status |
| `.Codex/skills/dev-execution/orchestration/workflow-patterns.md` | Pattern library |
| `.Codex/specs/workflows/schemas/execution-graph.schema.json` | args schema |
| `.Codex/specs/workflows/schemas/execution-report.schema.json` | return value schema |
| `.Codex/specs/provider-routing-spec.md` | Routing rules for agentType selection at pre-flight (scripts dispatch only; never resolve at runtime) |
