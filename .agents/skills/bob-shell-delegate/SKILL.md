...
description: Delegate bounded subtasks to IBM Bob Shell CLI. Use when the user wants to use Bob, delegate to Bob, run Bob for drafting/scaffolding/exploration, or compare Codex work with Bob output. Do NOT use for architecture-heavy integration work or cross-layer backend changes.
---
...
- Non-critical research or inventory tasks Codex will validate

### Bad fit — keep in Codex
...
## Single-Shot Only (from Codex)

Bob supports two session modes: interactive REPL (`bob`) and single-shot (`bob -p "..."`). **From Codex, only single-shot is viable.** The interactive REPL requires stdin input that Codex's Bash tool cannot provide mid-session.
...
For low-risk exploration while Codex owns integration.
...
This is exploratory only - Codex will make the final decision."
```

## Validation Checklist

After Bob completes, Codex must verify:
...
- **Reject** — validation failures on critical items; restart entirely in Codex
...
Alternatively, Codex can handle this task directly.
```
