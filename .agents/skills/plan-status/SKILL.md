...
- **Progress rollup**: A feature's progress is `completed` only when every progress file in its `.Codex/progress/<slug>*/` directory is done AND all tasks within those files are `completed`/`done`.
...
python .Codex/skills/artifact-tracking/scripts/manage-plan-status.py \
  --file .Codex/progress/<slug>/phase-N-progress.md --status completed
...
python .Codex/skills/artifact-tracking/scripts/update-batch.py \
  -f .Codex/progress/<slug>/phase-N-progress.md \
...
Surfaces `.Codex/findings/` files for archival:
...
Archival target: `.Codex/findings/archive/YYYY-MM/`
...
3. Check for progress files: `ls .Codex/progress/<feature_slug>/`
...
rm -r .Codex/progress/<slug>/  # if all files empty or planning-only
```
...
Surfaces the next actionable wave from every non-done meta-plan across `.Codex/plans/` and `docs/project_plans/meta-plans/`, plus in-progress implementation plans not already covered by a meta-plan wave.
...
| `manage-plan-status.py` | `.Codex/skills/artifact-tracking/scripts/` | Update plan/progress frontmatter fields |
| `update-batch.py` | `.Codex/skills/artifact-tracking/scripts/` | Batch-update task statuses in progress files |
| `update-status.py` | `.Codex/skills/artifact-tracking/scripts/` | Single task status update |
