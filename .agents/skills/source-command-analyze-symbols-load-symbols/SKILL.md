---
name: "source-command-analyze-symbols-load-symbols"
description: "Intelligently load relevant symbols based on task context using optimized chunked files"
---

# source-command-analyze-symbols-load-symbols

Use this skill when the user asks to run the migrated source command `analyze-symbols-load-symbols`.
...
```bash
# SkillMeat layered architecture: Routers → Services → Repositories → DB
