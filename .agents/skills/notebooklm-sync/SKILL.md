...
description: >-
  Deploy and manage NotebookLM documentation sync for any Codex project.
  Installs Python scripts + Codex hook that automatically syncs markdown
...
Deploy a NotebookLM documentation sync system into any Codex project. This skill packages Python scripts and a Codex hook that automatically syncs markdown documentation to a Google NotebookLM notebook whenever files are edited.
...
.Codex/hooks/
└── notebooklm-sync-hook.sh  # Codex PostToolUse hook
...
```bash
python .Codex/skills/notebooklm-sync/scripts/install.py \
  --project-name "<project-slug>" \
  --notebook-title "<title>" \
  --include-dirs "docs" \
  --include-dirs ".Codex/progress" \
...
3. Installs the Codex PostToolUse hook
...
```python
# Example: add .Codex/agents to scope
INCLUDE_DIRS = [
    "docs",
    ".Codex/progress",
    ".Codex/agents",  # <-- Added
...
```bash
python .Codex/skills/notebooklm-sync/scripts/uninstall.py --project-root .
```
...
2. Remove the Codex hook:
   ```bash
   rm -f .Codex/hooks/notebooklm-sync-hook.sh
   ```

3. Remove hook registration from `.Codex/settings.json`:
   ```json
   {
     "hooks": {
       // Remove this entry:
       "notebooklm-sync": {
         "trigger": "PostToolUse",
         "script": ".Codex/hooks/notebooklm-sync-hook.sh"
       }
     }
   }
   ```
...
1. The `.Codex/hooks/notebooklm-sync-hook.sh` PostToolUse hook fires
...
### Example 4: Add .Codex/agents to Scope

```bash
# Edit config
# In scripts/notebooklm_sync/config.py, add ".Codex/agents" to INCLUDE_DIRS
...
| Files not syncing on Write/Edit | Check that the hook is registered: `cat .Codex/settings.json \| grep notebooklm-sync`. Reinstall if missing: `python .Codex/skills/notebooklm-sync/scripts/install.py --repair`. |
...
| Hook script not executable | Run `chmod +x .Codex/hooks/notebooklm-sync-hook.sh`. |
| `jq` command not found | Install `jq`: `brew install jq` (macOS) or `apt install jq` (Linux). |

## Summary

This skill provides a complete NotebookLM documentation sync system for Codex projects. Use it to:
