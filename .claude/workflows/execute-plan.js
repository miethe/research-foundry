...
//
// ⚠️ THE SET BELOW IS A GENERATED REGION — a per-deployment fact inside a byte-identically
// deployed file. Those two properties cannot both hold, and the contradiction used to be resolved
// by hand-editing the block after each deploy while `check_global_artifact_drift.py` was told to
// `ignore_blocks` it and `sync_project_workflows.py` carried the local value across a write. An
// ignored region is not a reconciled one: a deliberate per-deployment value and a rotted mirror
// were indistinguishable from correct.
//
// They genuinely differ. Measured 2026-08-11 (node_01KZPH34N2BTBT2M7FT7A79WAH): `~/.claude/agents`
// holds 45 agents, `agentic_meta_dev/.claude/agents` holds 49, five of them present only in the
// latter. So one shared literal is wrong somewhere by construction — and it was. This file's
// pre-migration list named three agents resolvable in NO known agents dir (`frontend-developer`,
// `api-librarian`, `telemetry-auditor`) while omitting ~20 that do exist under `~/.claude/agents`;
// that is the invisible direction `hitl_routing: 'roster'` misclassifies on.
//
// So: regenerate per deployment with `scripts/gen_workflow_roster.py --root <root> --write`, and
// the drift gate now VERIFIES this region against that generator rather than eliding it. A hand
// edit is a gate failure instead of an invisible one. Do not hand-edit it, and do not reformat it
// — `--check` compares byte-for-byte, so a whitespace "improvement" reports drift in every
// deployment at once.
//
// The value carried HERE, upstream, is a SEED for a deployment that has not regenerated yet — not
// a claim about any deployment's agents. It is deliberately not generated: MeatySkills carries 4
// agent definitions, so generating from it would produce a roster small enough to disable
// roster-mode classification outright (the generator refuses below 10 for exactly that reason).
//
...
// >>> AOS-GENERATED KNOWN_AGENT_TYPES — regenerated by scripts/gen_workflow_roster.py
...
// <<< AOS-GENERATED KNOWN_AGENT_TYPES
