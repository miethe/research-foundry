...
decisions:
  ...
  - {decision: "Subprocess-per-agent-job isolation for the SDK adapters only; other static adapters unaffected (ADR-002) — notebooklm.py's pre-existing subprocess.run call is NOT re-architected by this ADR (SPIKE FU-5)", rationale: "Locked by SPIKE; live tool-use loop is a new risk class; notebooklm's existing unscoped subprocess is a separate, pre-existing exposure tracked as a follow-up, not reopened here", status: locked}
...
| D4: Subprocess isolation for SDK adapters only; other static adapters unaffected — notebooklm.py's pre-existing `subprocess.run` is NOT re-architected here (SPIKE FU-5) | ADR-002 (accepted) | locked |
