...
decisions:
  ...
  - {decision: "D12 nullable workspace_id/created_by become ENFORCED via a real data migration (dry-run + rollback)", rationale: "Columns exist (P2/P3 forward-compat); P5's job is migration+enforcement, not schema design", status: locked}
---
...
| D7: Enforce workspace_id/created_by via real migration (dry-run+rollback) | Columns exist (D12); job is migration+enforcement | locked |
...
| P5.2 | RBAC enforcement (5 roles, server-side) | Role→permission matrix; role checks on ALL mutations across catalog/reports/builder/runs/agents routers; enforce over `request.state.identity` (not UI hiding). R-P1 target_surfaces = enumerated routers. | Non-role mutation → 403; visibility enforced server-side | **Human gate #2** (RBAC-before-exposure sign-off) |
...
| P5.5 | Audit log | Append-only audit (durable store) of catalog mutations, report edits, agent launches, accepted artifacts, publish previews, writebacks — who/what/source/policy-snapshot. | Every mutating path emits an audit row (assertion test) | audit-coverage test green |
...
- **Severity**: high
- **Rationale**: spec §11 explicitly requires server-side enforcement; hiding in UI only is a public-release blocker.
- **Mitigation**: R-P1 enumerated router target_surfaces; per-route role assertion tests; Human gate #2 before exposure; Codex adversarial pass.
...
| P5.2 | 6 | RBAC across ~5 routers; H1 + cross-cutting enforcement |
| P5.3 | 6 | H3 migration/algorithmic flag; irreversible data migration + isolation |
| P5.4 | 5 | Clerk JWKS adapter + OIDC seam + FE login |
| P5.5 | 4 | Audit anchored to existing writeback/telemetry event plumbing |
| P5.6 | 5 | Rate-limit + admin + sharing gates (3 sub-areas) |
...
**Estimation Notes**: 46 pts is a large Tier-3 feature — the plan WILL exceed 800 lines and must
split into phase files. Bottom-up (46) is trusted over any "just add auth" top-down read (H4:
8 capability areas). If P5.4 Clerk defers to opt-in-later, P5 v1 floor is ~41.
