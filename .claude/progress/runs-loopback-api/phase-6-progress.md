...
tasks:
- id: TEST-001
  description: "TestClient test with fixture run corpus. Verify GET /api/runs response is list, each item has required RFRunSummary fields, empty corpus returns []."
  status: pending
  assigned_to:
  - python-backend-engineer
  dependencies:
...
- id: TEST-002
  description: "TestClient test for GET /api/runs/{run_id}: known run_id -> 200 + RFRunExport shape; unknown run_id -> 404 + {detail:...} structured JSON body."
  status: pending
  assigned_to:
  - python-backend-engineer
  dependencies:
...
- id: TEST-003
  description: "TestClient tests for GET /api/runs/{run_id}/claims (non-empty + empty array) and GET /api/runs/{run_id}/sources/{id} (found + 404). Verify redacted fields in claims response."
  status: pending
  assigned_to:
  - python-backend-engineer
  dependencies:
...
- id: TEST-004
  description: "TestClient test for GET /data/governance.json: response matches GovernanceConfig shape; no 500 on minimal config."
  status: pending
  assigned_to:
  - python-backend-engineer
  dependencies:
...
- id: TEST-005
  description: "TestClient test for GET /health: always returns 200 regardless of auth_mode (loopback + token)."
  status: pending
  assigned_to:
  - python-backend-engineer
  dependencies:
...
- id: TEST-006
  description: "Fixture run with sensitivity:work_sensitive claim. Threshold public. Verify GET /api/runs/{run_id} has quote and summary as [redacted:sensitivity]. Verify parity with direct export_service.export_run() call on same fixture."
  status: pending
  assigned_to:
  - python-backend-engineer
  dependencies:
  ...
  assigned_model: sonnet
  model_effort: adaptive
  ac_ref: "Parity assertion passes; redacted fields match exactly between API and direct export_service call. Risk R1 (sensitivity bypass)"

- id: TEST-007
  description: "TestClient with auth_mode=token: valid Bearer -> 200; missing header -> 401; invalid token -> 401; GET /health -> 200. Verify hmac.compare_digest path exercised."
  status: pending
  assigned_to:
  - python-backend-engineer
  dependencies:
...
- id: TEST-008
  description: "CliRunner test: rf serve --bind-host 0.0.0.0 --auth-mode none -> exits non-zero; --auth-mode token with token env unset -> exits non-zero; --auth-mode token with token set -> would start (stub uvicorn call)."
  status: pending
  assigned_to:
  - python-backend-engineer
  dependencies:
  ...
  assigned_model: sonnet
  model_effort: adaptive
  ac_ref: "Both failure cases exit non-zero; success case stubs uvicorn.run. Risk R2 (network exposure)"

- id: TEST-009
  description: "TestClient with non-empty allowlist: request from unlisted IP -> 403; request from listed IP -> 200."
  status: pending
  assigned_to:
  - python-backend-engineer
  dependencies:
...
- id: TEST-010
  description: "Core footprint isolation test: pip install research-foundry (no extra); python -c 'import research_foundry; print(ok)' succeeds; python -c 'import fastapi' fails."
  status: pending
  assigned_to:
  - python-backend-engineer
  dependencies:
...
- id: SC-P6-1
  ...
  status: pending
- id: SC-P6-2
  description: "Sensitivity-gate parity test (TEST-006) passes — most critical correctness gate"
  status: pending
- id: SC-P6-3
