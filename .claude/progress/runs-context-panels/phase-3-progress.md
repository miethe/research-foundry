...
tasks:
- id: P3S-001
  description: "Create 4 empty panel component files with prop types, empty-state\
    \ render, and collapsed-by-default shell (stubs only \u2014 no real data rendering).\
    \ Used for early type-checking while P2 runs."
  status: completed
  assigned_to:
  - ui-engineer-enhanced
  dependencies:
...
- id: P3-001
  description: 'Implement RoutingDecisionPanel.tsx: selected model profile, routing
    rationale, estimated cost vs budget, sensitivity tier. Collapsed by default. sessionStorage
    key rf:context-panel:${runId}:routing_decision. Empty-state for null prop (AC
    P2-R1).'
  status: completed
  assigned_to:
  - ui-engineer-enhanced
  ...
  model_effort: adaptive
- id: P3-002
  description: 'Implement ResearchBriefPanel.tsx: wrap existing ReportMarkdownRenderer
    for context.research_brief_md. Strip YAML frontmatter before passing to renderer.
    Collapsed by default. Empty-state for null prop (AC P2-R2).'
  status: completed
  assigned_to:
  - frontend-developer
  dependencies:
  ...
  model_effort: adaptive
- id: P3-003
  description: "Implement SwarmPlanPanel.tsx: two-level collapsible list tree (adapters\
    \ \u2192 steps), cost columns, 3-level depth cap, raw-YAML 'Show raw' escape hatch\
    \ for unrecognized shapes. SwarmPlanNode type per OQ-3. Empty-state for null prop\
    \ (AC P2-R3)."
  status: completed
  assigned_to:
  - ui-engineer-enhanced
  ...
  model_effort: adaptive
- id: P3-004
  description: 'Implement UpstreamEntitiesPanel.tsx: render intent_id, ibom_id, intenttree_node_id
    as styled badge links (online) or plain-text badges with tooltip (offline). Best-effort
    service ping; never blocks render. Empty-state for null prop (AC P2-R4).'
  status: completed
  assigned_to:
  - frontend-developer
  dependencies:
  ...
  model_effort: adaptive
- id: P3-005
  description: 'Wire all 4 panels into RunDetailWorkspace.tsx. Each panel receives
    context.* field via optional chaining (no hard destructuring). Order: Routing
    Decision, Research Brief, Swarm Plan, Upstream Entities. sessionStorage collapse
    state per run+panel.'
  status: completed
  assigned_to:
  - ui-engineer-enhanced
  ...
  model_effort: adaptive
- id: P3-006
  description: "Add schema_version guard in frontend: schema_version >= 1.3 \u2192\
    \ panels active; lower/absent \u2192 all panels show 'Context not available for\
    \ this run' empty-state. No regressions in existing 1.2 consumers."
  status: completed
  assigned_to:
  - frontend-developer
  dependencies:
  ...
  model_effort: adaptive
- id: TEST-SMOKE
  description: "Runtime smoke across all 4 panel surfaces: (1) fully populated context\
    \ run; (2) pre-1.3 context-absent run (context: null) \u2014 all panels empty-state,\
    \ no JS errors; (3) offline static build \u2014 SPA built without rf serve, panels\
    \ functional from run.json alone."
  status: completed
  assigned_to:
  - ui-engineer-enhanced
  dependencies:
...
- id: SC-P3-6
  ...
  status: pending
- id: SC-P3-7
  description: TEST-SMOKE runtime smoke passes (all 3 scenarios including offline
    build and pre-1.3 run)
  status: pending
- id: SC-P3-8
