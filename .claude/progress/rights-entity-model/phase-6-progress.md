...
owners: ["python-backend-engineer", "documentation-writer", "changelog-generator"]
contributors: []
...
tasks:
  ...
  - id: P6-2
    ...
    description: "Consolidate P3-4 + P5-2 into one CI-run cross-check confirming both halves of the §9.10 boundary are covered end-to-end in a single suite run (regression guard against either negative test being accidentally skipped/marked xfail in isolation)."
    status: pending
    assigned_to: ["python-backend-engineer"]
    dependencies: ["P6-1"]
  ...
  - id: P6-3
    ...
    description: "Cross-phase integration test tying P1's taxonomy (judgment_basis) to P5-3's gate — exercises the full pipeline (capture → taxonomy assignment → release-gate evaluation) rather than P5-3's unit-level bidirectional test."
    status: pending
    assigned_to: ["python-backend-engineer"]
    dependencies: ["P6-1"]
    estimated_effort: "0.5 pts"
  ...
  - id: P6-4
    ...
    description: "Finalize P2-3's unit-level reproducibility test at the integration level: two full rf rights validate --as-of <date> CLI invocations against the same corpus produce byte-identical stdout; monkeypatch wall-clock calls to raise, run through the CLI (not just the service function directly)."
    status: pending
    assigned_to: ["python-backend-engineer"]
    dependencies: ["P6-1"]
    estimated_effort: "0.5 pts"
    ...
    model_effort: adaptive
  - id: P6-5
    title: "Fixture regeneration + §9.9 note"
    description: "Regenerate RF's example source_card/source_assertion fixtures (inline-dict test fixtures — RF has no checksum files) to include rights_summary/taxonomy/synthesis fields where applicable. Document explicitly that §9.9's role-string-in-reviewer-field defect is moot for RF — RF ports schema structure only, never the pediatric-repo's vendored JSON example files, so there is no RF-side instance of that defect to correct."
    status: pending
    assigned_to: ["python-backend-engineer"]
    dependencies: ["P6-1"]
  ...
  - id: P6-6
    ...
    description: "Add entry under [Unreleased] per .claude/specs/changelog-spec.md categorization rules. changelog_required: true in this plan's frontmatter — mandatory, not skippable."
    status: pending
  ...
  - id: P6-8a
    ...
    description: "Author docs/project_plans/design-specs/rights-runtime-resolution-api.md (maturity: idea, prd_ref set to this feature's PRD) capturing the OQ-3/DI-RIGHTS-1 alternative (a fast runtime resolution API vs. the mirror this plan ships) and when it would be worth building. Append path to this plan's deferred_items_spec_refs."
    status: pending
    assigned_to: ["documentation-writer"]
    dependencies: ["P5-4"]
    estimated_effort: "0.5 pts"
  ...
  - id: P6-8b
    ...
    description: "Author docs/project_plans/design-specs/rights-surveillance-loop.md capturing OQ-RF-5/DI-RIGHTS-3 (a scheduled re-check loop against next_review_at). Append path to deferred_items_spec_refs."
    status: pending
    assigned_to: ["documentation-writer"]
    dependencies: ["P5-4"]
    estimated_effort: "0.5 pts"
  ...
  - id: P6-8c
    ...
    description: "Author docs/project_plans/design-specs/rights-counsel-workflow.md capturing OQ-RF-6/DI-RIGHTS-4 (a formal rights-owner/counsel role + attestation workflow — currently no such role exists upstream; shipped as human-only-by-exclusion). Append path to deferred_items_spec_refs."
    status: pending
    assigned_to: ["documentation-writer"]
    dependencies: ["P5-4"]
    estimated_effort: "0.5 pts"
  ...
  - id: P6-9
    ...
    description: "Set status: completed, populate commit_refs/files_affected/updated, confirm deferred_items_spec_refs has all 3 P6-8 paths, confirm adr_refs includes P5-4's ADR path."
    status: pending
    assigned_to: ["documentation-writer"]
    dependencies: ["P6-6", "P6-7", "P6-8a", "P6-8b", "P6-8c"]
    estimated_effort: "0.5 pts"
    priority: "low"
    assigned_model: sonnet
    model_effort: adaptive
...
parallelization:
  batch_1: ["P6-1"]
  batch_2: ["P6-2", "P6-3", "P6-4", "P6-5"]
  batch_3: ["P6-6", "P6-7", "P6-8a", "P6-8b", "P6-8c"]
  batch_4: ["P6-9"]
  critical_path: ["P6-1", "P6-6", "P6-9"]
  estimated_total_time: "7 pts (plan bottom-up estimate)"

blockers: []

success_criteria:
  - {id: "P6-SC1", description: "Enum-consistency, negative-write-path, release-gate, and determinism test sweeps all green", status: pending}
  - {id: "P6-SC2", description: "Fixture regeneration complete; §9.9 moot-determination documented", status: pending}
  - {id: "P6-SC3", description: "CHANGELOG [Unreleased] entry present", status: pending}
  - {id: "P6-SC4", description: "README/docs/context files/artifact-type-reference updated", status: pending}
  - {id: "P6-SC5", description: "All 3 DOC-006 design specs authored and appended to deferred_items_spec_refs", status: pending}
  - {id: "P6-SC6", description: "Plan frontmatter finalized (status, commit_refs, files_affected, deferred_items_spec_refs, adr_refs)", status: pending}
  - {id: "P6-SC7", description: "Reviewer gate: karen end-of-feature (final sign-off across all 7 phases — explicit verdict required, silence is never a pass)", status: pending}

notes: |
  §9.9 note: this phase owns the one remaining unassigned §9 adjudication row (§9.9, P6-5).
  Dependencies on "All P0-P5 phases" (P6-6, P6-7) are represented here as the 6 phase IDs
  (P0-P5) rather than individual task IDs, per the phase plan's literal dependency text.
  changelog-generator and documentation-writer default to haiku in this environment and hard-error —
  always override to sonnet (see .claude/rules and decisions-block Model Routing Notes).
  karen's verdict at the end of this phase is the feature's final sign-off across all 7 phases —
