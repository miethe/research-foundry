...
Generates comprehensive API reference documentation from OpenAPI/AsyncAPI specifications, following the Diátaxis documentation structure and integrating with the SkillMeat docs system.
...
generate_redoc_docs() {
  ...
  if command -v redoc-cli >/dev/null 2>&1; then
    ...
    redoc-cli build "$spec_file" --output "$output_dir/index.html" --title "SkillMeat API Reference"
...
<html>
<head>
  <title>SkillMeat API Reference</title>
...
Complete technical reference for the SkillMeat API.

## Quick Reference

- **Base URL**: `https://api.SkillMeat.com`
- **API Version**: `v1`
...
- **Status Page**: [status.SkillMeat.com](https://status.SkillMeat.com)
EOF
}
```
