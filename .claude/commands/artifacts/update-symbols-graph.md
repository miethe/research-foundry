...
Analyze SkillMeat codebase for symbol extraction:

```bash
# SAFETY: Validate we're in the SkillMeat project
if [[ ! -f "pnpm-workspace.yaml" ]] || [[ ! -d "services/api" ]]; then
  echo "ERROR: Not in SkillMeat project directory" >&2
  exit 1
fi

# Check for recent SkillMeat code changes only
git diff --name-only HEAD~1 HEAD | \
  grep -E '\.(ts|tsx|py|js|jsx)$' | \
  grep -E '^(apps|services|packages)/' || echo "No recent code changes in project directories"

# Count SkillMeat source files by language (exclude third-party)
echo "SkillMeat TypeScript files: $(find apps services packages -name '*.ts' -o -name '*.tsx' 2>/dev/null | grep -v node_modules | grep -v dist | wc -l)"
echo "SkillMeat Python files: $(find services -name '*.py' 2>/dev/null | grep -v __pycache__ | wc -l)"
echo "SkillMeat JavaScript files: $(find apps packages -name '*.js' -o -name '*.jsx' 2>/dev/null | grep -v node_modules | grep -v dist | wc -l)"

# List SkillMeat source directories only
...
## SkillMeat Project Boundaries

**CRITICAL**: This command ONLY analyzes SkillMeat project source code files. It must exclude third-party code, generated files, and non-source files.

### Inclusion Rules (ONLY these files)
- **Directories**: `apps/`, `services/`, `packages/` (SkillMeat project structure)
- **Extensions**: `.ts`, `.tsx`, `.py`, `.js`, `.jsx` (source code only)
- **Location**: Files within the main project directories that are part of the SkillMeat codebase
...
```bash
# STEP 1: Validate we're in SkillMeat project root
if [[ ! -f "pnpm-workspace.yaml" ]] || [[ ! -d "services/api" ]] || [[ ! -d "packages/ui" ]]; then
  echo "ERROR: Not in SkillMeat project root directory" >&2
  exit 1
fi

# STEP 2: Find ONLY SkillMeat project source files with explicit filtering
...
echo "Analyzing $FILE_COUNT SkillMeat project source files..."
```
...
```bash
# Function to validate file is part of SkillMeat project
...
1. **Validate file** belongs to SkillMeat project (not third-party)
...
```bash
# Full SkillMeat symbol graph generation
/update-symbols-graph

# Dry run to preview output (validates project structure first)
/update-symbols-graph --dry-run

# Only analyze TypeScript files in SkillMeat project
/update-symbols-graph --language=ts

# Update specific SkillMeat directory
/update-symbols-graph apps/web/src/components

# Focus on changed SkillMeat files only (excludes third-party)
...
This command implements strict filtering to ensure it ONLY processes SkillMeat project code:

1. **Directory Validation**: Must execute from SkillMeat root (validates `pnpm-workspace.yaml` exists)
...
- ❌ Scan outside the SkillMeat project structure
...
ls pnpm-workspace.yaml services/api packages/ui apps/web >/dev/null 2>&1 && echo "✅ SkillMeat project detected" || echo "❌ Not in SkillMeat root"
