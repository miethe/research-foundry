...
- Structural spec-compliance pass on the already-shipped SKILL.md against
  `.claude/skills/_meta/skill-authoring-guide.md`:
  ...
  - Key References converted from relative to absolute paths so the skill remains loadable
    from arbitrary working directories (it is symlinked into `~/.claude/skills/`).
...
- `.agents/skills/rf-knowledge/` Codex-host mirror added in lockstep — same file list, with the
  standard host-agent token rewrite (`Claude Code` → `Codex`, host-agent `Claude` → `Codex`,
  `CLAUDE.md` → `AGENTS.md`, `.claude/` → `.Codex/`). Domain nouns (`Anthropic`, `SkillMeat`,
  `MeatyWiki`, `Research Foundry`, the `rf` / `op` CLI names, `rf-knowledge`,
  `rf_knowledge_lookup`, `claude-sonnet-*` / `claude-opus-*` model ids) preserved.
