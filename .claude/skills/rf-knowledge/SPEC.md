...
owner: "Nick Miethe"
source_docs:
  - .claude/skills/rf-knowledge/SKILL.md
  - .claude/skills/rf-knowledge/references/tool-reference.md
  - .claude/skills/rf-knowledge/references/gotchas.md
...
- Structural remediation of an already-shipped SKILL.md against
  `.claude/skills/_meta/skill-authoring-guide.md`: added `version` / `app_version` / `updated` /
...
|-----------------|--------------------|-------|
| `rf_knowledge_lookup` subagent | Delegated by an orchestrator that needs to keep recall out of its own context | Answers a bounded question against the governed RF corpus; returns an answer + opaque ids and never writes, acquires, or mints evidence. Definition: `.claude/agents/research/rf_knowledge_lookup.md`. |
