...
Full project scaffold — analyzes project context and assembles a Bundle of domain-specific artifacts covering all identified capabilities. **Invoked manually from Claude Code after project initialization and Project Starters deployment.** Builds on top of the baseline artifacts deployed by `skillmeat scaffold --full` or the New Project dialog.

## Context: When to Use This Mode

This mode is typically used **after** a new project has been set up via:

- `skillmeat scaffold --full` (CLI) — runs init + deploys Project Starters + prints hint to run `/scaffold:project`
- New Project dialog (web UI) — creates project + deploys starters + shows callout with `/scaffold:project` command

**Example workflow**:
1. Developer runs `skillmeat scaffold --full` in an empty directory
2. Command prompts for name, description, platforms, and spec file location
3. Command deploys the org's Project Starters (e.g., planning skills, devops agents, debug context)
4. Command prints: `Done. Open Claude Code and run /scaffold:project to add domain-specific artifacts.`
5. Developer opens Claude Code and invokes `/scaffold:project` in the project directory
6. This skill analyzes the PRD/directory and recommends additional artifacts (async task queues, API testing utilities, etc.) tailored to the project's tech stack
7. User confirms and deploys the recommended Bundle on top of the starters
...
The `project-analyzer` subagent reads the input and extracts an Intent Set YAML. This analysis is scoped to **domain-specific capabilities not already covered by Project Starters**. The Project Starters baseline (planning skills, debug context, etc.) is assumed to be already deployed; this phase focuses on recommendations for the project's unique tech stack and architecture.
