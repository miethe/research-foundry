"""ClaudeAgentSDKProvider — ResearchAgentProvider backed by AgentJobService subprocesses.

Implements the full :class:`ResearchAgentProvider` Protocol for the
``claude_agent_sdk`` backend, wrapping the subprocess-isolation boundary
...
callable that returns ``b"test-mock-key-stub"``.  All test and offline usage
MUST rely on this default or a custom factory that similarly returns stub bytes.
Real API credential factories MUST NOT be wired in until Gate #2 is approved.
...
_TEST_CREDENTIAL_BYTES = b"test-mock-key-stub"
...
class ClaudeAgentSDKProvider(BaseProvider):
    """ResearchAgentProvider for the ``claude_agent_sdk`` backend.
    ...
    credential_bytes_factory:
        ...
        that returns ``b"test-mock-key-stub"`` (safe for offline / CI use).

        **Constraint**: MUST NOT return real API credentials until Mode-D
        Gate #2 is approved.  Never read ``ANTHROPIC_API_KEY`` or any other
    ...
    def start_job(self, job: dict[str, Any]) -> str:
        ...
        mode this returns ``b"test-mock-key-stub"``.
        ...
        research_foundry.services.clearance.ClearanceDenied
            ...
            (or ``acquisition``) refuses the whole spawn. ``spawn_job`` below
            hands the job to a child process that drives the third-party SDK, so
            this is the last in-process point before source text leaves the
            machine on this path — and ``AgentJob`` has a fixed field list that
            drops any stamp, so mediating the raw mapping (not the constructed
            ``AgentJob``) is what design invariant 4 requires.
        """
        svc_paths = getattr(self._svc, "_paths", None)
        mediate_prompt_egress(
            job,
            target=f"agent_provider:{self.id}",
            # An injected mock service may carry any object here; only a real
            # FoundryPaths is usable for registry resolution (None falls back to
            # FoundryPaths.discover(), which is the pre-existing behaviour of
            # every other clearance call site).
        ...
        logger.info("ClaudeAgentSDKProvider: started job %s", job_id)
        return job_id
    ...
    def accept_artifacts(
        ...
        logger.info(
            "ClaudeAgentSDKProvider: accepted artifacts %s for job %s",
            accepted_now,
            job_id,
        )
...
register(ClaudeAgentSDKProvider())
