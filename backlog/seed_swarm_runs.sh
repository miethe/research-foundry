...
rf capture 'How can a draft report be reliably decomposed into discrete material claims and each claim aligned to a supporting source card (or labeled inference/speculation) so the claim verifier'\''s hard gates can be enforced?' --from manual --sensitivity personal --tag research-foundry --tag claim-verification --tag attribution --tag entailment --tag claim-ledger
...
rf capture 'What does the empirical literature say about unsupported-claim/hallucination rates in multi-step LLM synthesis, what existing frameworks implement claim-to-source traceability, and how does a claim-ledger + verifier architecture compare to RAG, constitutional AI, and self-consistency for reducing material unsupported claims?' --from manual --sensitivity personal --tag research-foundry --tag claim-verification --tag hallucination --tag verifiable-ai --tag rag

# RIB-003 [high] Contradiction detection across heterogeneous source cards
#   -> plan: --depth deep --audience technical --max-cost 7 --freshness 180d  (artifact: technical_memo)
rf capture 'What NLP/LLM approaches most effectively detect factual contradictions between extracted evidence points across heterogeneous source types (web, PDF, papers, notes) for building a contradiction_log, and how should LLM-maintained knowledge graphs flag stale/orphaned/conflicting claims at acceptable precision and cost?' --from manual --sensitivity personal --tag research-foundry --tag contradiction-detection --tag evidence-quality --tag knowledge-graph --tag fact-checking
...
#   -> plan: --depth standard --audience technical --max-cost 5 --freshness 180d  (artifact: technical_memo)
rf capture 'What methods do existing research/evidence-synthesis systems use to detect, flag, and re-validate stale or superseded sources over time, and how do they apply to RF'\''s source-card model and 180-day default freshness window?' --from manual --sensitivity personal --tag research-foundry --tag source-freshness --tag stale-data --tag source-cards --tag knowledge-decay
...
#   -> plan: --depth standard --audience technical --max-cost 5 --freshness 180d  (artifact: technical_memo)
rf capture 'Given intake-time features (artifact type, length, structural richness, source hint), which lightweight classification architectures predict the optimal LLM tier with sub-5ms overhead, and what minimum labeled dataset size is needed for reliable calibration?' --from manual --sensitivity personal --tag research-foundry --tag llm-routing --tag classification --tag cost-optimization --tag model-selection
...
rf capture 'What published security frameworks, threat models, and runtime-enforcement mechanisms address work/personal API-key isolation and cross-profile credential leakage in multi-profile agentic systems, where have prior implementations failed, and how do they validate or improve RF'\''s four-profile key-governance model?' --from manual --sensitivity personal --tag research-foundry --tag key-isolation --tag governance --tag credential-security --tag boundary

# RIB-019 [medium] Content-level secret and PII scanning as a writeback runtime gate
#   -> plan: --depth standard --audience technical --max-cost 5 --freshness 180d  (artifact: technical_memo)
rf capture 'How should RF detect secrets and PII inside report/evidence content and block or redact before writeback, as a runtime gate distinct from key isolation?' --from manual --sensitivity personal --tag agentic-os --tag governance --tag secret-scanning --tag pii --tag writeback
...
#   -> plan: --depth deep --audience technical --max-cost 7 --freshness 180d  (artifact: technical_memo)
rf capture 'What patterns (file-watch hooks, post-merge triggers, SSE, event sourcing) most effectively close manual cross-tool handoff seams (research→vault, vault→graph, telemetry→governance) in a local-first agentic OS without introducing a centralized message broker?' --from manual --sensitivity personal --tag research-foundry --tag event-driven --tag seam-integration --tag local-first --tag file-watch
...
rf capture 'Under what corpus size, query-type distribution, and update frequency does a Karpathy-style compiled persistent wiki produce measurably better answer quality, latency, and cost than standard RAG over the same raw sources, and what scaling limits do community implementations reveal?' --from manual --sensitivity personal --tag research-foundry --tag llm-wiki --tag rag --tag knowledge-management --tag benchmarks

# RIB-037 [medium] Markdown/Git vs DB-as-truth for reproducible AI knowledge artifacts
#   -> plan: --depth standard --audience technical --max-cost 5 --freshness 365d  (artifact: technical_memo)
rf capture 'What are the documented trade-offs between file-first (Markdown+Git) and database-first (Postgres/SQLite/vector/graph) architectures for LLM-maintained knowledge bases — around reproducibility, diffability, schema evolution, concurrent writes, agent tool compatibility, human editability, and query performance at scale?' --from manual --sensitivity personal --tag research-foundry --tag file-first --tag database --tag architecture --tag git
